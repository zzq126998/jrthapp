...
$(function(){
	...
	$(".addresslist").delegate("a", "click", function(){
		...
		$("#addressid").val(id);
		$(".t .address-info").html('<span class="name">'+name+'</span><span class="tel">'+tel+'</span><span class="address-txt">'+addr+'</span>').removeClass('empty');
		$("#p2 .goback1").click();
	});
	...
	function ordPrice(){
		...
		var or_price=parseInt($(".shop-info-price span i").text());//商品单价
