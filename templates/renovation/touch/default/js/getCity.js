    ...
    var gzAddress         = $(".gz-address"),  //选择地址页
        ...
        gzAddrInit = {
            ...
            ,getAddrArea: function(id){
                ...
                $.ajax({
                    url: masterDomain + "/include/ajax.php?service=renovation&action=addr",
                    data: "type="+id,
                    type: "GET",
                    dataType: "jsonp",
    ...
    gzSelAddrList.delegate("li", "click", function(){
        ...
        if(id && addr){
            ...
            //没有下级
                ...
                gzAddrSeladdr.removeClass("gz-no-sel").attr("data-id", id).find(".city").addClass('curr').html(addrname[addrname.length-1]);
                gzAddrSeladdr.find(".city").val(addrname[addrname.length-1]);
                gzAddrInit.hideNewAddrMask();
                $('#addr').val(id);
