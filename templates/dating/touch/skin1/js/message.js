$(function(){
  ...
  function getList(tr){
    ...
    $.ajax({
      url: masterDomain+'/include/ajax.php?service=dating&action=review',
      type: 'get',
      dataType: 'jsonp',
      success: function(data){
        if(data && data.state == 100){
          ...
          for(var i = 0; i < data.info.list.length; i++){
            ...
            html.push('<li class="item'+(d.isread == 0 ? ' is_new' : '')+(d.user.level > 0 ? ' vip' : '')+(d.info.isread == "0" ? ' is_new' : '')+'" data-id="'+d.info.id+'" data-type="'+d.user.type+'" data-my="'+d.info.my+'" data-other="'+d.user.id+'">');
            html.push('  <a href="javascript:;" class="fn-clear">');
            html.push('    <div class="visitors_img">');
            html.push('      <div class="img"><img src="'+d.user.phototurl+'"></div>');
            html.push('    </div>');
            html.push('    <div class="visitors_txt">');
            html.push('      <p class="txt_name">');
            html.push('        <em>'+d.user.nickname+'</em>');
