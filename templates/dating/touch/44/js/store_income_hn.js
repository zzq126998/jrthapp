$(function(){
  ...
  function getList(tr){
    ...
    $.ajax({
      url: masterDomain + '/include/ajax.php?service=dating&action=incomeList&type=1&hnid='+id+'&spec=in&autoread=1&page='+page+'&pageSize='+pageSize+'&category='+category+'&keywords='+keywords,
      type: 'get',
      dataType: 'jsonp',
      success: function(data){
        ...
        if(data && data.state == 100){
          ...
          if(len){
            $(".hn_total .money font").text(data.info.pageInfo.totalMoney);
            for(var i = 0; i < len; i++){
              var d = list[i];
