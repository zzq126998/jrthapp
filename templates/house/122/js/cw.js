$(function () {
    ...
    function getList() {
        ...
        var url = "/include/ajax.php?service=house&action=cwList";
                ...
                if (data.state == 100) {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        html.push('<a href="' + d.url + '" target="_blank"><h2>' + d.title + '</h2>' + (d.isbid == "1" ?
                            '<i class="mtop"></i>' : '') + '</a>');
                        html.push('<span class="lpprice">' + (d.price > 0 ? '<b>' + d.price + '</b>' + (d.type == '1' ? '万'+echoCurrency('short')+'' : ''+echoCurrency('short')+'/月') : '<b>价格面议</b>') +
                            '</span>');
                        html.push('</div>');
                        ...
                        if(d.type == 0){
                            if(d.price > 0){
                                html.push('<div class="sp_r fn-right">' + Math.ceil(d.price / d.area) + ' '+echoCurrency('short')+'/月·平方</div>');
                            }
                        }else if (d.type == "1") {
                            if(d.price > 0){
