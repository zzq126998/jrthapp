$(function(){
  ...
  $('#addNav').click(function(){
    ...
    html.push('    <input type="text" class="clearBoxSize title" value="" placeholder="输入标题">');
    html.push('    <input type="text" class="clearBoxSize url" value="" placeholder="输入链接地址">');
    html.push('  </div>');
  ...
  function updateMenuEdit(detail){
    ...
    if($.trim(detail.body)){
      ...
      $("#content").html('<div class="textarea placeholder" contenteditable="true"><font class="txt-gray">请输入内容</font></div>');
      ...
      $('.content .textarea').blur(function(){
        var t = $(this), txtGray = t.find('.txt-gray');
        if (t.html() == "") {
          t.html('<font class="txt-gray">请输入内容</font>');
        }
      })
    }
      ...
      success: function(data){
        if(data && data.state == 100){
          ...
          for(var i = 0; i < data.info.length; i++){
            var d = data.info[i];
            html.push('<li data-id="'+d.id+'">'+d.title+'<span class="del">删除</span><span class="edit">编辑</span></li>');
          }
  ...
  $("#content").blur(function(){
    var t = $(this), con = t.html();
    if(con == ""){
      t.addClass("placeholder").html("请输入内容");
    }
  });
