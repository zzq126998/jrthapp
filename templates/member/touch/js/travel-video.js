$(function () {
    ...
    // 删除
    $(".video-list").delegate(".del", "click", function(){
        var t = $(this), par = t.closest(".hotel-box"), id = par.attr("data-id");
        if(id){
            var result = confirm(langData['siteConfig'][20][211]);
            if(result){
                $.ajax({
                    url: masterDomain+"/include/ajax.php?service=travel&action=operVideo&oper=del&id="+id,
                    type: "GET",
                    dataType: "jsonp",
    ...
    function getList(tr){
        ...
        $.ajax({
            url: masterDomain+"/include/ajax.php?service=travel&action=videoList&u=1&orderby=2&state="+state+"&page="+atpage+"&pageSize="+pageSize,
            type: "GET",
            dataType: "jsonp",
            success: function (data) {
                isload = false;
                if(data && data.state == 100){
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        html.push('<dd class="label_info">');
                        html.push('<p class="left_info"><i><img src="'+huoniao.changeFileSize(list[i].user['photo'], "small")+'" /></i><span>'+list[i].user['nickname']+'</span></p>');
                        html.push('<p class="right_info">'+langData['travel'][11][33]+'：'+list[i].click+'</p>');
                        html.push('</dd>');
                        html.push('</a></dl>');
                        html.push('</div>');
