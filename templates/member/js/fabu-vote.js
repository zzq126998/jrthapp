$(function(){
  ...
  obj.delegate(".xuan_del", "click", function(){
    ...
    if(len <= 1){
      $.dialog.alert(langData['siteConfig'][30][99]);   //至少保留两个选项
      return;
    }else{
      delPic(li);
      checkXuan(ul);
  ...
  var addCustomInput = function(where){
    ...
    var getbody = function(n){
      ...
      body.push('      <a href="javascript:;" class="icon icon_down xuan_down" title="'+langData['siteConfig'][6][1598]+'"'+(n == 2 ? ' style="display:none;"' : '')+'></a>'); //下移
    ...
    if(where == 'question'){
      ...
      html.push('          <li data-id="0">'+langData['siteConfig'][31][84]+'</li>');
      ...
      html.push('        <span>'+langData['siteConfig'][31][76]+'</span>');  //单选题
