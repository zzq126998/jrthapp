$(function () {
    ...
    // 删除
    $(".agency-list").delegate(".del", "click", function(){
        var t = $(this), par = t.closest(".hotel-box"), id = par.attr("data-id");
        if(id){
            var result = confirm(langData['siteConfig'][20][211]);
            if(result){
                $.ajax({
                    url: masterDomain+"/include/ajax.php?service=travel&action=operAgency&oper=del&id="+id,
                    type: "GET",
                    dataType: "jsonp",
    ...
    function getList(tr){
        ...
        $.ajax({
            url: masterDomain+"/include/ajax.php?service=travel&action=agencyList&u=1&orderby=2&state="+state+"&page="+atpage+"&pageSize="+pageSize,
            type: "GET",
            dataType: "jsonp",
            success: function (data) {
                isload = false;
                if(data && data.state == 100){
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        html.push('<dd class="price">');
                        html.push('<p class="right_text"><span>'+echoCurrency('symbol')+'<em>'+list[i].price+'</em></span>'+langData['travel'][1][6]+'</p>');
                        html.push('<p class="left_text">'+list[i].travelservice+'</p>');
                        html.push('</dd>');
                        html.push('</a></dl>');
                        html.push('</div>');
