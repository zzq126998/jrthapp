$(function(){
  ...
  HN_Location.init(function(data){
    if (data == undefined || data.address == "" || data.name == "" || data.lat == "" || data.lng == "") {
      $('.list.load').removeClass('load').children('.loader').text(langData['siteConfig'][27][137]);
    }else{
      lng = data.lng;
      lat = data.lat;
    ...
    $.ajax({
      ...
      success: function(data){
        if(data.state == 100){
          ...
          $('.business .list').removeClass('load');
        }
      },
      error: function(){
        $(".business .list").removeClass("load").find(".loader").text(langData['siteConfig'][20]['183']);
      }
    });
  }
  ...
  function getWaimaiShop(){
    $.ajax({
      ...
      success: function(data){
          ...
          if(data.state == 100){
              ...
              for(var i = 0; i < info.length; i++){
                  ...
                  if(d.is_discount == '1'){
                    list.push('        <li><span class="zhe">'+langData['waimai'][2][91]+'</span>'+langData['waimai'][2][8].replace('1', d.discount_value)+'</li>');
                  }
      ...
      error: function(){
        $(".waimai .list").removeClass("load").find(".loader").text(langData['siteConfig'][20]['183']);
      }
  })
  }
})
