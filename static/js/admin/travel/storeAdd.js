...
$(function(){
	...
	function checkGw(t, val, id){
		...
		huoniao.operaJson("../inc/json.php?action=checkUser_travelstore", "key="+val+"&id="+id, function(data){
			t.removeClass("input-loading");
			if(data == 200){
				t.siblings(".input-tips").removeClass().addClass("input-tips input-error").html('<s></s>此会员已授权管理其它旅游公司，一个会员不可以管理多个旅游公司！');
			}else{
				if(data) {
					for(var i = 0; i < data.length; i++){
				...
				if(flag){
					t.siblings(".input-tips").removeClass().addClass("input-tips input-ok").html('<s></s>如果填写了，则此会员可以管理旅游公司');
				}else{
