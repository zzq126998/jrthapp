$(function(){
  ...
  $(".subway").bind("click", function(event){
	...
	$(".addPrice").bind("click", function(){
		...
		html = html.replace(/#id/g, n).replace(/#k/g, n);
		var d = $(html);
		d.addClass('group group_'+n).attr('data-id', n);
		$(this).closest('table').find("tbody:eq(0)").append(d);
	});
	//删除套餐
	$(".priceWrap table").delegate(".delGroup", "click", function(){
		var t = $(this), tr = t.closest('tr'), id = tr.attr('data-id');
		$.dialog.confirm("确定要删除吗？", function(){
			$('.group_'+id+', .it_'+id).remove();
		});
	});

	// 增加价格
	$(".priceWrap table").delegate(".add", "click", function(){
		var t = $(this), td = t.parent('td'), tr = td.parent('tr'), id = tr.attr('data-id');

		var html = $("#trTemp").html();
		var n = getTimes();
		html = html.replace(/#id/g, id).replace(/#k/g, n);
		var d = $(html);
		d.addClass('it it_'+id).attr("data-id", id);

		var obj = tr;
		while(obj.next().length && obj.next().hasClass('it_'+id)){
			obj = obj.next();
		}
		obj.after(d);

		$('.it_'+id).find('.td_name, .td_oper').addClass('hide').find('input').prop('disabled', true);

		var rowspan = $('.it_'+id).length + 1;
		$('.group_'+id).find('.td_name, .td_oper').attr('rowspan', rowspan);
		
	})
	//删除价格
	$(".priceWrap table").delegate(".del", "click", function(){
		var t = $(this), td = t.parent('td'), tr = t.closest("tr"), id = tr.attr('data-id'), isgroup = tr.hasClass('group'), next = tr.next();
		if(isgroup){
				if(!next.hasClass('it')){
					$.dialog.alert('至少保留一项价格配置');
					return;
				}
		}
		...
		$.dialog.confirm("确定要删除吗？", function(){
			...
			var ntd = null;
			if(isgroup){
				next.removeClass().addClass('group group_'+id);
			}

			$('.it_'+id).find('.td_name, .td_oper').addClass('hide').find('input').prop('disabled', true);
			var rowspan = $('.it_'+id).length + 1;
			$('.group_'+id).find('.td_name, .td_oper').attr('rowspan', rowspan).find('input').prop('disabled', false);
