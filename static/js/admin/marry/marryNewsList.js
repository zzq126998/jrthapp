$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryNewsList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		var href = $(this).attr("href"), type = "marry";
		try {
			event.preventDefault();
			parent.addPage("marryNewsListphpdopostAdd", type, "新增婚尚资讯", "marry/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("marryNewsEdit"+id, "marry", title, "marry/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("marryNewsList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, marryNewsList = val.marryNewsList;
		if(val.state == "100"){
			huoniao.hideTip();

			obj.attr("data-totalpage", val.pageInfo.totalPage);

			for(i; i < marryNewsList.length; i++){
				list.push('<tr data-id="'+marryNewsList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left"><a href="'+marryNewsList[i].url+'" target="_blank">'+marryNewsList[i].title+'</a></td>');
				list.push('  <td class="row5">'+marryNewsList[i].weight+'</td>');
				list.push('  <td class="row15"><a href="javascript:;" data-id="'+marryNewsList[i].typeid+'" class="type">'+marryNewsList[i].type+'</a></td>');
				list.push('  <td class="row15">'+marryNewsList[i].date+'</td>');
				list.push('  <td class="row12">'+marryNewsList[i].state+'</td>');
				list.push('  <td class="row15"><a data-id="'+marryNewsList[i].id+'" data-title="'+marryNewsList[i].title+'" href="marryNewsList.php?dopost=Edit&id='+marryNewsList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
