$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryHotel.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryHotel.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("marryHotelAdd", "marry", "添加酒店", "marry/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".modify", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("marryHotelEdit"+id, "marry", title, "marry/"+href);
		} catch(e) {}
	});

	//删除
	$("#delBtn").bind("click", function(){
		$.dialog.confirm('确定后将删除此酒店的所有信息<br />此操作不可恢复，您确定要删除吗？', function(){
			init.del();
		});
	});
	...
	//宴会厅
	$("#list").delegate(".hall", "click", function(event){
		var id = $(this).parent().parent().attr("data-id"),
			title = $(this).attr("data-title");

		try {
			event.preventDefault();
			parent.addPage("marryHotelHall"+id, "marry", "宴会厅-"+title, "marry/marryHotelHall.php?hotelid="+id);
		} catch(e) {}
	});

	//菜单
	$("#list").delegate(".menu", "click", function(event){
		var id = $(this).parent().parent().attr("data-id"),
			title = $(this).attr("data-title");

		try {
			event.preventDefault();
			parent.addPage("marryHotelMenu"+id, "marry", "菜单-"+title, "marry/marryHotelMenu.php?hotelid="+id);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("marryHotel.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, marryHotel = val.marryHotel;
		...
		if(val.state == "100"){
			...
			for(i; i < marryHotel.length; i++){
				list.push('<tr data-id="'+marryHotel[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row25 left"><img src="'+cfg_attachment+marryHotel[i].litpic+'&type=small" width="100" class="litpic" /><a href="'+marryHotel[i].url+'" target="_blank">'+marryHotel[i].title+'</a></td>');
				list.push('  <td class="row10 left"><a href="javascript:;" data-id="'+marryHotel[i].userid+'" class="userinfo">'+marryHotel[i].username+'</a></td>');
				list.push('  <td class="row10 left">'+marryHotel[i].levaltype+'</td>');
				list.push('  <td class="row15 left">['+marryHotel[i].addr+']'+marryHotel[i].address+'</td>');
				list.push('  <td class="row12 left">'+marryHotel[i].pubdate+'</td>');
				var state = "";
				switch (marryHotel[i].state) {
					case "0":
				...
				list.push('<a data-id="'+marryHotel[i].id+'" data-title="'+marryHotel[i].title+'" href="marryHotel.php?dopost=edit&id='+marryHotel[i].id+'" title="修改" class="link modify">修改</a>&nbsp;|&nbsp;');
				list.push('<a href="javascript:;" title="宴会厅" data-title="'+marryHotel[i].title+'" class="link hall">宴会厅</a>&nbsp;|&nbsp;');
				list.push('<a href="javascript:;" title="菜单" data-title="'+marryHotel[i].title+'" class="link menu">菜单</a>&nbsp;|&nbsp;');
