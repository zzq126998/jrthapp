...
$(function(){
	...
	$("#zjcom").bind("input", function(){
		...
		if(val != ""){
			...
			huoniao.operaJson("../inc/json.php?action=checkMarryStore", "key="+val+"&filter=6", function(data){
	...
	$("#btnSubmit").bind("click", function(event){
		...
		if(hostname==''){
			$("#hostname").siblings(".input-tips").removeClass().addClass("input-tips input-error").attr("style", "display:inline-block");
			huoniao.goTop();
			return false;
		}
		...
		//异步提交
		huoniao.operaJson("marryhostAdd.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "save"){
					huoniao.parentTip("success", "婚嫁主持人添加成功！<a href='"+data.url+"' target='_blank'>"+data.url+"</a>");
					huoniao.goTop();
					location.reload();
				}else{
					huoniao.parentTip("success", "婚嫁主持人修改成功！<a href='"+data.url+"' target='_blank'>"+data.url+"</a>");
