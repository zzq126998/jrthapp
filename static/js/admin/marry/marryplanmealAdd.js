$(function(){
	...
	//开始、结束、有效时间
	$("#holdingtime").datetimepicker({format: 'yyyy-mm-dd hh:ii:ss', autoclose: true, minView: 0, language: 'ch'});
	...
	$("#zjcom").bind("input", function(){
		...
		if(val != ""){
			...
			huoniao.operaJson("../inc/json.php?action=checkMarryStore", "key="+val+"&filter="+filter, function(data){
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//异步提交
		huoniao.operaJson("marryplanmealAdd.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "save"){
					huoniao.parentTip("success", "婚嫁套餐添加成功！<a href='"+data.url+"' target='_blank'>"+data.url+"</a>");
					huoniao.goTop();
					location.reload();
				}else{
					huoniao.parentTip("success", "婚嫁套餐修改成功！<a href='"+data.url+"' target='_blank'>"+data.url+"</a>");
