$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryRitual.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryRitual.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("marryRitualAdd", "marry", "添加婚庆公司", "marry/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".modify", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("marryRitualEdit"+id, "marry", title, "marry/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".sale", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("marryRitualSale"+id, "marry", "精品折扣-"+title, "marry/marrySale.php?action=ritual&cid="+id);
		} catch(e) {}
	});
	...
	$("#list").delegate(".deal", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("marryRitualDeal"+id, "marry", "优惠活动-"+title, "marry/marryDeal.php?action=ritual&cid="+id);
		} catch(e) {}
	});
	...
	$("#list").delegate(".coupon", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("marryRitualCoupon"+id, "marry", "电子优惠券-"+title, "marry/marryCoupon.php?action=ritual&cid="+id);
		} catch(e) {}
	});
	...
	$("#list").delegate(".works", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("marryRitualWorks"+id, "marry", "作品精选-"+title, "marry/marryWorks.php?action=ritual&cid="+id);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("marryRitual.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, marryRitual = val.marryRitual;
		...
		if(val.state == "100"){
			...
			for(i; i < marryRitual.length; i++){
				list.push('<tr data-id="'+marryRitual[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row25 left"><img src="'+cfg_attachment+marryRitual[i].litpic+'" width="100" class="litpic" /><a href="'+marryRitual[i].url+'" target="_blank">'+marryRitual[i].title+'</a></td>');
				list.push('  <td class="row10 left"><a href="javascript:;" data-id="'+marryRitual[i].userid+'" class="userinfo">'+marryRitual[i].username+'</a></td>');
				list.push('  <td class="row10 left">'+marryRitual[i].levaltype+'</td>');
				list.push('  <td class="row15 left">['+marryRitual[i].addr+']'+marryRitual[i].address+'</td>');
				list.push('  <td class="row12 left">'+marryRitual[i].pubdate+'</td>');
				var state = "";
				switch (marryRitual[i].state) {
					case "0":
				...
				list.push('<a data-id="'+marryRitual[i].id+'" data-title="'+marryRitual[i].title+'" href="marryRitual.php?dopost=edit&id='+marryRitual[i].id+'" title="修改" class="link modify">修改</a>&nbsp;|&nbsp;');
				...
				list.push('<li><a href="javascript:;" class="sale" data-id="'+marryRitual[i].id+'" data-title="'+marryRitual[i].title+'">精品折扣</a></li>');
				list.push('<li><a href="javascript:;" class="deal" data-id="'+marryRitual[i].id+'" data-title="'+marryRitual[i].title+'">优惠活动</a></li>');
				list.push('<li><a href="javascript:;" class="coupon" data-id="'+marryRitual[i].id+'" data-title="'+marryRitual[i].title+'">电子优惠券</a></li>');
				list.push('<li><a href="javascript:;" class="works" data-id="'+marryRitual[i].id+'" data-title="'+marryRitual[i].title+'">作品精选</a></li>');
				list.push('</ul></span>');
				list.push('</td>');
				list.push('</tr>');
			}
