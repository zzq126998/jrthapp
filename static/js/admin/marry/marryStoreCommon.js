		...
		init = {
			...
			,quickEdit: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryStoreCommon.php?dopost=getCommonDetail", "id="+id, function(data){
						if(data != null && data.length > 0){
							...
							$.dialog({
								...
								ok: function(){
									...
									huoniao.operaJson("marryStoreCommon.php?dopost=updateCommonDetail", "id="+id+"&typeid="+typeid+"&"+serialize, function(data){
										if(data.state == 100){
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryStoreCommon.php?dopost=delCommon", "id="+id+"&typeid="+typeid, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryStoreCommon.php?dopost=updateCommonState", "id="+id+"&typeid="+typeid+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
...
function getList(){
	...
	huoniao.operaJson("marryStoreCommon.php?dopost=getCommonList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < commonList.length; i++){
				...
				list.push('  <td class="row35 title left">标题：<a href="'+commonList[i].articleUrl+'" target="_blank">'+commonList[i].articleTitle+'</a><br />内容：'+commonList[i].commonContent+'</td>');
