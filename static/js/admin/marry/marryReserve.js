		...
		init = {
			...
			,quickEdit: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryReserve.php?dopost=getDetail&action="+action, "id="+id, function(data){
						if(data != null && data.length > 0){
							...
							$.dialog({
								...
								ok: function(){
									...
									huoniao.operaJson("marryReserve.php?dopost=updateDetail&action="+action, "id="+id+"&"+serialize, function(data){
										if(data.state == 100){
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryReserve.php?dopost=delReserve&action="+action, "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryReserve.php?dopost=updateState&action="+action, "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
...
function getList(){
	...
	huoniao.operaJson("marryReserve.php?dopost=getList&action="+action, data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, ReserveList = val.ReserveList;
		...
		if(val.state == "100"){
			...
			for(i; i < ReserveList.length; i++){
				list.push('<tr data-id="'+ReserveList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row10 left">'+ReserveList[i].people+'</td>');
				list.push('  <td class="row12 left">'+ReserveList[i].contact+'</td>');
				list.push('  <td class="row25 left">'+ReserveList[i].aname+'</td>');
				list.push('  <td class="row13 left">'+ReserveList[i].usertime+'</td>');
				var state = "";
				switch (ReserveList[i].state) {
					case "0":
				...
				list.push('  <td class="row25"><a href="javascript:;" data-id='+ReserveList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" data-id='+ReserveList[i].id+'" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
