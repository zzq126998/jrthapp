$(function(){
	...
	$("#zjcom").bind("input", function(){
		...
		if(val != ""){
			...
			huoniao.operaJson("../inc/json.php?action=checkMarryStore", "key="+val+"&filter=2", function(data){
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//异步提交
		huoniao.operaJson("marryplancaseAdd.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "save"){
					huoniao.parentTip("success", "商家案例添加成功！<a href='"+data.url+"' target='_blank'>"+data.url+"</a>");
					huoniao.goTop();
					location.reload();
				}else{
					huoniao.parentTip("success", "商家案例修改成功！<a href='"+data.url+"' target='_blank'>"+data.url+"</a>");
