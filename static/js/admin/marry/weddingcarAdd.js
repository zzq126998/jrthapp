$(function(){
	...
	$("#zjcom").bind("input", function(){
		...
		if(val != ""){
			...
			huoniao.operaJson("../inc/json.php?action=checkMarryStore", "key="+val+"&filter=3", function(data){
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//异步提交
		huoniao.operaJson("weddingcarAdd.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "save"){
					huoniao.parentTip("success", "婚车添加成功！<a href='"+data.url+"' target='_blank'>"+data.url+"</a>");
					huoniao.goTop();
					location.reload();
				}else{
					huoniao.parentTip("success", "婚车修改成功！<a href='"+data.url+"' target='_blank'>"+data.url+"</a>");
