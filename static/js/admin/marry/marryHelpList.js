$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryHelpList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("marryHelpListphpdopostAdd", type, "新增婚尚帮助", "marry/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("marryHelpEdit"+id, "marry", title, "marry/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("marryHelpList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, marryHelpList = val.marryHelpList;
		if(val.state == "100"){
			huoniao.hideTip();

			obj.attr("data-totalpage", val.pageInfo.totalPage);

			for(i; i < marryHelpList.length; i++){
				list.push('<tr data-id="'+marryHelpList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left"><a href="'+marryHelpList[i].url+'" target="_blank">'+marryHelpList[i].title+'</a></td>');
				list.push('  <td class="row5">'+marryHelpList[i].weight+'</td>');
				list.push('  <td class="row15"><a href="javascript:;" data-id="'+marryHelpList[i].typeid+'" class="type">'+marryHelpList[i].type+'</a></td>');
				list.push('  <td class="row15">'+marryHelpList[i].date+'</td>');
				list.push('  <td class="row12">'+marryHelpList[i].state+'</td>');
				list.push('  <td class="row15"><a data-id="'+marryHelpList[i].id+'" data-title="'+marryHelpList[i].title+'" href="marryHelpList.php?dopost=Edit&id='+marryHelpList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
