		...
		init = {
			...
			,quickEdit: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryCommon.php?dopost=getDetail&action="+action, "id="+id, function(data){
						if(data != null && data.length > 0){
							...
							$.dialog({
								...
								ok: function(){
									...
									huoniao.operaJson("marryCommon.php?dopost=updateDetail&action="+action, "id="+id+"&"+serialize, function(data){
										if(data.state == 100){
							...
							self.parent.$("#userInfo").html('<a href="javascript:;" data-id="'+data.userid+'">'+data.nickname+'</a>');
							self.parent.$("#level").html(data.level+"分");
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryCommon.php?dopost=delCommon&action="+action, "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryCommon.php?dopost=updateState&action="+action, "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
...
function getList(){
	...
	huoniao.operaJson("marryCommon.php?dopost=getList&action="+action, data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < CommonList.length; i++){
				...
				list.push('  <td class="row25 left">'+CommonList[i].aname+'</td>');
				list.push('  <td class="row15 left"><a href="javascript:;" data-id="'+CommonList[i].userid+'" class="userinfo link">'+CommonList[i].username+'</a></td>');
				list.push('  <td class="row12 left">'+CommonList[i].level+' 分</td>');
				list.push('  <td class="row13 left">'+CommonList[i].pubdate+'</td>');
				var state = "";
				switch (CommonList[i].state) {
					case "0":
				...
				list.push('  <td class="row25"><a href="javascript:;" data-id='+CommonList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" data-id='+CommonList[i].id+'" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
