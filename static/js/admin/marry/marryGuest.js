		...
		init = {
			...
			,quickEdit: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryGuest.php?dopost=getDetail&action="+action, "id="+id, function(data){
						if(data != null && data.length > 0){
							...
							$.dialog({
								...
								ok: function(){
									...
									huoniao.operaJson("marryGuest.php?dopost=updateDetail&action="+action, "id="+id+"&"+serialize, function(data){
										if(data.state == 100){
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryGuest.php?dopost=delGuest&action="+action, "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryGuest.php?dopost=updateState&action="+action, "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
...
function getList(){
	...
	huoniao.operaJson("marryGuest.php?dopost=getList&action="+action, data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < guestList.length; i++){
				...
				list.push('  <td class="row25 left">'+guestList[i].aname+'</td>');
