		...
		init = {
			...
			,quickEdit: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("videoCommon.php?dopost=getDetail", "id="+id, function(data){
						if(data != null && data.length > 0){
							...
							$.dialog({
								...
								ok: function(){
									...
									huoniao.operaJson("videoCommon.php?dopost=updateDetail", "id="+id+"&"+serialize, function(data){
										if(data.state == 100){
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("videoCommon.php?dopost=delComment", "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("videoCommon.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
...
function getList(){
	...
	huoniao.operaJson("videoCommon.php?dopost=getList", data.join("&"), function(val){
