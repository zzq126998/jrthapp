$(function(){
	...
	$("#extractlTab input").keyup(function(){
		var t = $(this), val = t.val() == '' ? 0 : t.val(), p = t.closest('tr');
		var total = 0;
		p.find("input").not('.pt').each(function(){
			var r = $(this).val();
			r = r == '' ? 0 : parseInt(r);
			if(!isNaN(r)){
				total += r;
	...
	$("#btnSubmit").bind("click", function(event){
		...
		if(type == "site"){
			...
			var leadPrice = [], no = 0;
			$(".leadPrice").remove();
			$("#leadPriceTab tr").each(function(i){
				var tr = $(this), month = $.trim(tr.find('.month').val()), price = $.trim(tr.find('.price').val()), count = $.trim(tr.find('.count').val());
				if(month != '' && price != ''){
					month = parseInt(month);
					price = parseFloat(price);
					count = parseInt(count);

					if(!isNaN(month) && !isNaN(price) && !isNaN(count)){
						$(".priceWrap").after('<input type="hidden" class="leadPrice" name="leadPrice['+no+'][month]" value="'+month+'" />');
						$(".priceWrap").after('<input type="hidden" class="leadPrice" name="leadPrice['+no+'][price]" value="'+price+'" />');
						$(".priceWrap").after('<input type="hidden" class="leadPrice" name="leadPrice['+no+'][count]" value="'+count+'" />');
						no ++ ;
					}
				}
			})

			$("#extractlTab select, #extractlTab .not").attr("disabled", false);
		...
		if(type == "about"){
			$("#extractlTab select, #extractlTab .not").attr("disabled", true);
		}
