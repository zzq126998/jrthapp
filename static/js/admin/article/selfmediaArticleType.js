$(function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					if(jsonArray["parentid"] == 0){
						...
						typeList.push('  <div class="row20 left"><input type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'" class="typename">'+addType+'</div>');
						typeList.push('  <div class="row20 left"><input type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["jump"]+'" class="jump"></div>');
					}else{
						typeList.push('  <div class="row3"></div>');
						typeList.push('  <div class="row20 left"><span class="plus-icon" style="margin-left:'+cl+'px;"></span><input type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'" class="typename">'+addType+'</div>');
						typeList.push('  <div class="row20 left"><span class="plus-icon" style="margin-left:'+cl+'px;"></span><input type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["jump"]+'" class="jump"></div>');
					}
					...
					typeList.push('  <div class="row17 left"><a href="javascript:;" class="del" title="删除">删除</a></div>');
					typeList.push('</div>');
	...
	$("#addNew_").bind("click", function(){
		...
		html.push('    <div class="row60 left"><input data-id="0" type="text" value="" class="typename"></div>');
		html.push('    <input data-id="0" type="hidden" value="" class="jump">');
		html.push('    <input data-id="0" type="hidden" value="" class="icon">');
	...
	$("#list").delegate("input", "blur", function(){
		var id = $(this).attr("data-id"), value = $(this).val(), name = $(this).attr("class");
		if(id != "" && id != 0){
			huoniao.operaJson("?dopost=updateType&id="+id+"&action="+action, "type=single&name="+name+"&val="+value, function(data){
				if(data.state == 100){
					huoniao.showTip("success", data.info, "auto");
				}else if(data.state == 101){
	...
	$(".root").delegate(".del", "click", function(event){
		...
		var t = $(this), id = t.parent().parent().find("input").attr("data-id"), type = t.closest('tr').find('.typename').attr('data-id');
...
function saveOpera(type){
	...
	for(var i = 0; i < first.length; i++){
		(function(){
			var html =arguments[0], count = 0, jArray = $(html).find(">ul>li"), tr = $(html).find(".tr .typename"), id = tr.attr("data-id"), val = tr.val(), icon = $(html).find(".tr .icon").val(), jump = $(html).find(".tr .jump").val();

			if(jArray.length > 0 && val != ""){
				json = json + '{"id": "'+id+'", "name": "'+encodeURIComponent(val)+'", "icon": "'+icon+'", "jump": "'+jump+'", "lower": [';
				for(var k = 0; k < jArray.length; k++){
					if($(jArray[k]).find(">ul>li").length > 0){
						arguments.callee(jArray[k]);
					}else{
						var tr = $(jArray[k]).find(".tr .typename"), id = tr.attr("data-id"), val = tr.val(), jump = $(html).find(".tr .jump").val();
						icon = $(jArray[k]).find(".tr .icon").val();
						if(val != ""){
							json = json + '{"id": "'+id+'", "name": "'+encodeURIComponent(val)+'", "icon": "'+icon+'", "jump": "'+jump+'", "lower": null},';
						}else{
							count++;
						}
					}
	...
	huoniao.operaJson("?dopost=typeAjax", "aid="+aid+"&data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
