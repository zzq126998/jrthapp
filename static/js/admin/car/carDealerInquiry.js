$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("carDealerInquiry.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(e){
		...
		try {
			e.preventDefault();
			parent.addPage("carDealerInquiryAdd", "car", "新增询价信息", "car/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(e){
		...
		try {
			e.preventDefault();
			parent.addPage("carDealerInquiryEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("carDealerInquiry.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, carDealerInquiry = val.carDealerInquiry;
		...
		if(val.state == "100"){
			...
			for(i; i < carDealerInquiry.length; i++){
				list.push('<tr data-id="'+carDealerInquiry[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left"><a href="/carDealer.php?id='+carDealerInquiry[i].aid+'" target="_blank">'+carDealerInquiry[i].aName+'</a></td>');
				list.push('  <td class="row15 left"><a href="/carParam.php?id='+carDealerInquiry[i].cid+'" target="_blank">'+carDealerInquiry[i].cName+'</a></td>');
				list.push('  <td class="row15 left">'+carDealerInquiry[i].people+'</td>');
				list.push('  <td class="row15 left">'+carDealerInquiry[i].contact+'</td>');
				list.push('  <td class="row12 left">'+carDealerInquiry[i].addr+'</td>');
				list.push('  <td class="row13 left">'+carDealerInquiry[i].pubdate+'</td>');
				list.push('  <td class="row12">');
				list.push('<a data-id="'+carDealerInquiry[i].id+'" data-title="修改询价信息" href="carDealerInquiry.php?dopost=edit&id='+carDealerInquiry[i].id+'" title="修改" class="edit">修改</a>');
