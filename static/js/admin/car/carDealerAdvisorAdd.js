$(function(){
	...
	thumbnail = function() {
		new SWFUpload({
			upload_url: "/include/upload.inc.php?mod="+modelType+"&type=photo&filetype=image",
	...
	$("#btnSubmit").bind("click", function(e){
		...
		//异步提交
		huoniao.operaJson("carDealerAdvisor.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "Add"){
					$.dialog({
						fixed: true,
					...
					$.dialog({
						...
						ok: function(){
							try{
								$("body",parent.document).find("#nav-carDealerAdvisorphp").click();
								parent.reloadPage($("body",parent.document).find("#body-carDealerAdvisorphp"));
								$("body",parent.document).find("#nav-carDealerAdvisorEdit"+id+" s").click();
							}catch(e){
								location.href = thisPath + "carDealerAdvisor.php";
							}
						},
						cancel: false
					});
