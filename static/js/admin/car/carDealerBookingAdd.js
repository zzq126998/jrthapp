$(function(){
	...
	$("#btnSubmit").bind("click", function(e){
		...
		//异步提交
		huoniao.operaJson("carDealerBooking.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "Add"){
					$.dialog({
						fixed: true,
					...
					$.dialog({
						...
						ok: function(){
							try{
								$("body",parent.document).find("#nav-carDealerBookingphp").click();
								parent.reloadPage($("body",parent.document).find("#body-carDealerBookingphp"));
								$("body",parent.document).find("#nav-carDealerBookingEdit"+id+" s").click();
							}catch(e){
								location.href = thisPath + "carDealerBooking.php";
							}
						},
						cancel: false
					});
