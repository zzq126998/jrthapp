$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("carWiki.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carWikiphpdopostAdd", type, "新增汽车百科", "car/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carWikiEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("carWiki.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, carWiki = val.carWiki;
		if(val.state == "100"){
			huoniao.hideTip();
			
			obj.attr("data-totalpage", val.pageInfo.totalPage);
			
			for(i; i < carWiki.length; i++){
				list.push('<tr data-id="'+carWiki[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left">'+carWiki[i].title+'</td>');
				list.push('  <td class="row5">'+carWiki[i].weight+'</td>');
				list.push('  <td class="row15"><a href="javascript:;" data-id="'+carWiki[i].typeid+'" class="type">'+carWiki[i].type+'</a></td>');
				list.push('  <td class="row15">'+carWiki[i].date+'</td>');
				list.push('  <td class="row12">'+carWiki[i].state+'</td>');
				list.push('  <td class="row15"><a data-id="'+carWiki[i].id+'" data-title="'+carWiki[i].title+'" href="carWiki.php?dopost=Edit&id='+carWiki[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
