$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("carFagui.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carFaguiphpdopostAdd", type, "新增汽车法规", "car/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carFaguiEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("carFagui.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, carFagui = val.carFagui;
		if(val.state == "100"){
			huoniao.hideTip();
			
			obj.attr("data-totalpage", val.pageInfo.totalPage);
			
			for(i; i < carFagui.length; i++){
				list.push('<tr data-id="'+carFagui[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left">'+carFagui[i].title+'</td>');
				list.push('  <td class="row5">'+carFagui[i].weight+'</td>');
				list.push('  <td class="row15"><a href="javascript:;" data-id="'+carFagui[i].typeid+'" class="type">'+carFagui[i].type+'</a></td>');
				list.push('  <td class="row15">'+carFagui[i].date+'</td>');
				list.push('  <td class="row12">'+carFagui[i].state+'</td>');
				list.push('  <td class="row15"><a data-id="'+carFagui[i].id+'" data-title="'+carFagui[i].title+'" href="carFagui.php?dopost=Edit&id='+carFagui[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
