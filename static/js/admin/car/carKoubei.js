$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("carKoubei.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carKoubeiAdd", "car", "添加新口碑", "car/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carKoubeiEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("carKoubei.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, carKoubei = val.carKoubei;
		...
		if(val.state == "100"){
			...
			for(i; i < carKoubei.length; i++){
				list.push('<tr data-id="'+carKoubei[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left">'+carKoubei[i].title+'</td>');
				list.push('  <td class="row10 left"><a href="javascript:;" data-id="'+carKoubei[i].uid+'" class="userinfo" title="查看会员信息">'+carKoubei[i].username+'</a></td>');
				list.push('  <td class="row15 left">'+carKoubei[i].cName+'</td>');
				list.push('  <td class="row15 left">'+carKoubei[i].paramName+'</td>');
				list.push('  <td class="row10 left">'+carKoubei[i].fuel+'L/100km</td>');
				list.push('  <td class="row12 left">'+carKoubei[i].pubdate+'</td>');
				list.push('  <td class="row15">');
				list.push('<a data-id="'+carKoubei[i].id+'" data-title="'+carKoubei[i].title+'" href="carKoubei.php?dopost=edit&id='+carKoubei[i].id+'" title="修改" class="edit">修改</a>');
