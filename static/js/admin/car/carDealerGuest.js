$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("carDealerGuest.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(e){
		...
		try {
			e.preventDefault();
			parent.addPage("carDealerGuestAdd", "car", "新增留言信息", "car/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(e){
		...
		try {
			e.preventDefault();
			parent.addPage("carDealerGuestEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("carDealerGuest.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, carDealerGuest = val.carDealerGuest;
		...
		if(val.state == "100"){
			...
			for(i; i < carDealerGuest.length; i++){
				list.push('<tr data-id="'+carDealerGuest[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><a href="/carDealer.php?id='+carDealerGuest[i].aid+'" target="_blank">'+carDealerGuest[i].aName+'</a></td>');
				list.push('  <td class="row20 left"><a href="/carParam.php?id='+carDealerGuest[i].cid+'" target="_blank">'+carDealerGuest[i].cName+'</a></td>');
				list.push('  <td class="row20 left">'+carDealerGuest[i].contact+'</td>');
				list.push('  <td class="row12 left">'+carDealerGuest[i].addr+'</td>');
				list.push('  <td class="row13 left">'+carDealerGuest[i].pubdate+'</td>');
				list.push('  <td class="row12">');
				list.push('<a data-id="'+carDealerGuest[i].id+'" data-title="修改留言信息" href="carDealerGuest.php?dopost=edit&id='+carDealerGuest[i].id+'" title="修改" class="edit">修改</a>');
