$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("carDealerEvent.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carDealerEventAdd", "car", "添加新活动", "car/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carDealerEventEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("carDealerEvent.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, carDealerEvent = val.carDealerEvent;
		...
		if(val.state == "100"){
			...
			for(i; i < carDealerEvent.length; i++){
				list.push('<tr data-id="'+carDealerEvent[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><img src="'+cfg_attachment+carDealerEvent[i].litpic+'&type=small" width="100" class="litpic" /><a href="/carDealerEvent.php?id='+carDealerEvent[i].id+'" target="_blank">'+carDealerEvent[i].title+'</a></td>');
				list.push('  <td class="row15 left">'+carDealerEvent[i].cName+'</td>');
				list.push('  <td class="row15 left">'+carDealerEvent[i].aName+'</td>');
				list.push('  <td class="row12 left">'+carDealerEvent[i].start+'</td>');
				list.push('  <td class="row12 left">'+carDealerEvent[i].end+'</td>');
				list.push('  <td class="row13 left">'+carDealerEvent[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+carDealerEvent[i].id+'" data-title="修改促销活动" href="carDealerEvent.php?dopost=edit&id='+carDealerEvent[i].id+'" title="修改" class="edit">修改</a>');
