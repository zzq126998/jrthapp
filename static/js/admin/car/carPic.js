$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("carPic.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carPicAdd", "car", "添加新图片", "car/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carPicEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("carPic.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, carPic = val.carPic;
		...
		if(val.state == "100"){
			...
			for(i; i < carPic.length; i++){
				list.push('<tr data-id="'+carPic[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><img src="'+cfg_attachment+carPic[i].pic+'&type=small" width="100" class="litpic" /><a href="/carPic.php?id='+carPic[i].id+'" target="_blank">'+carPic[i].title+'</a></td>');
				list.push('  <td class="row20 left">'+carPic[i].cName+'</td>');
				list.push('  <td class="row20 left">'+carPic[i].paramName+'</td>');
				list.push('  <td class="row10 left">'+carPic[i].type+'</td>');
				list.push('  <td class="row12 left">'+carPic[i].pubdate+'</td>');
				list.push('  <td class="row15">');
				list.push('<a data-id="'+carPic[i].id+'" data-title="'+carPic[i].title+'" href="carPic.php?dopost=edit&id='+carPic[i].id+'" title="修改" class="edit">修改</a>');
