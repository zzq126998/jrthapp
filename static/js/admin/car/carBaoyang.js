$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("carBaoyang.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carBaoyangAdd", "car", "添加新养护", "car/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carBaoyangEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("carBaoyang.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, carBaoyang = val.carBaoyang;
		...
		if(val.state == "100"){
			...
			for(i; i < carBaoyang.length; i++){
				list.push('<tr data-id="'+carBaoyang[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left">'+carBaoyang[i].cName+'</td>');
				list.push('  <td class="row20 left">'+carBaoyang[i].paramName+'</td>');
				list.push('  <td class="row15 left">'+carBaoyang[i].shoubao+'公里</td>');
				list.push('  <td class="row15 left">'+carBaoyang[i].baoyang+'公里</td>');
				list.push('  <td class="row12 left">'+carBaoyang[i].pubdate+'</td>');
				list.push('  <td class="row15">');
				list.push('<a data-id="'+carBaoyang[i].id+'" data-title="修改养护信息" href="carBaoyang.php?dopost=edit&id='+carBaoyang[i].id+'" title="修改" class="edit">修改</a>');
