$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("carDealerAdvisor.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(e){
		...
		try {
			parent.addPage("carDealerAdvisorAdd", "car", "添加新顾问", "car/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(e){
		...
		try {
			e.preventDefault();
			parent.addPage("carDealerAdvisorEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
	...
	//单选
	$("#list tbody").delegate("tr", "click", function(Advisor){
		var isCheck = $(this), checkLength = $("#list tbody tr.selected").length;
		if(Advisor.target.className.indexOf("check") > -1) {
			if(isCheck.hasClass("selected")){
				isCheck.removeClass("selected");
			}else{
				isCheck.addClass("selected");
			}
		}else if(Advisor.target.className.indexOf("edit") > -1 || Advisor.target.className.indexOf("del") > -1) {
			$("#list tr").removeClass("selected");
			isCheck.addClass("selected");
		}else{
			if(checkLength > 1){
...
function getList(){
	...
	huoniao.operaJson("carDealerAdvisor.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, carDealerAdvisor = val.carDealerAdvisor;
		...
		if(val.state == "100"){
			...
			for(i; i < carDealerAdvisor.length; i++){
				list.push('<tr data-id="'+carDealerAdvisor[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><img src="'+cfg_attachment+carDealerAdvisor[i].litpic+'&type=small" width="45" /> '+carDealerAdvisor[i].name+'</td>');
				list.push('  <td class="row20 left">'+carDealerAdvisor[i].duty+'</td>');
				list.push('  <td class="row30 left"><a href="/carDealer.php?id='+carDealerAdvisor[i].aid+'" target="_blank">'+carDealerAdvisor[i].aName+'</a></td>');
				list.push('  <td class="row15 left">'+carDealerAdvisor[i].pubdate+'</td>');
				list.push('  <td class="row12">');
				list.push('<a data-id="'+carDealerAdvisor[i].id+'" data-title="修改促销顾问" href="carDealerAdvisor.php?dopost=edit&id='+carDealerAdvisor[i].id+'" title="修改" class="edit">修改</a>');
