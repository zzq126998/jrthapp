$(function(){
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("carVideoType.php?dopost=updateType&id="+id, "type=single&typename="+value, function(data){
				if(data.state == 100){
	...
	$(".root").delegate(".del", "click", function(event){
		...
		if(t.parent().parent().next("ul").html() != undefined && t.parent().parent().next("ul").html() != ""){
			...
			if(type.indexOf("编辑") > -1){
				$.dialog.confirm("确定后，此分类下的信息也将同时删除，请谨慎操作！", function(){
					huoniao.operaJson("carVideoType.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								location.reload();
...
function saveOpera(type){
	...
	huoniao.operaJson("carVideoType.php?dopost=typeAjax", "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
