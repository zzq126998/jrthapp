$(function(){
	...
	$("#btnSubmit").bind("click", function(e){
		...
		//异步提交
		huoniao.operaJson("carDealerInquiry.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "Add"){
					$.dialog({
						fixed: true,
					...
					$.dialog({
						...
						ok: function(){
							try{
								$("body",parent.document).find("#nav-carDealerInquiryphp").click();
								parent.reloadPage($("body",parent.document).find("#body-carDealerInquiryphp"));
								$("body",parent.document).find("#nav-carDealerInquiryEdit"+id+" s").click();
							}catch(e){
								location.href = thisPath + "carDealerInquiry.php";
							}
						},
						cancel: false
					});
