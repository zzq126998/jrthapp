$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("carDealerBooking.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(e){
		...
		try {
			e.preventDefault();
			parent.addPage("carDealerBookingAdd", "car", "新增试驾信息", "car/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(e){
		...
		try {
			e.preventDefault();
			parent.addPage("carDealerBookingEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("carDealerBooking.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, carDealerBooking = val.carDealerBooking;
		...
		if(val.state == "100"){
			...
			for(i; i < carDealerBooking.length; i++){
				list.push('<tr data-id="'+carDealerBooking[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left"><a href="/carDealer.php?id='+carDealerBooking[i].aid+'" target="_blank">'+carDealerBooking[i].aName+'</a></td>');
				list.push('  <td class="row15 left"><a href="/carParam.php?id='+carDealerBooking[i].cid+'" target="_blank">'+carDealerBooking[i].cName+'</a></td>');
				list.push('  <td class="row15 left">'+carDealerBooking[i].people+'</td>');
				list.push('  <td class="row15 left">'+carDealerBooking[i].contact+'</td>');
				list.push('  <td class="row12 left">'+carDealerBooking[i].addr+'</td>');
				list.push('  <td class="row13 left">'+carDealerBooking[i].pubdate+'</td>');
				list.push('  <td class="row12">');
				list.push('<a data-id="'+carDealerBooking[i].id+'" data-title="修改试驾信息" href="carDealerBooking.php?dopost=edit&id='+carDealerBooking[i].id+'" title="修改" class="edit">修改</a>');
