$(function(){
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("carFaguiType.php?dopost=updateType&id="+id, "type=single&typename="+value, function(data){
				if(data.state == 100){
	...
	$(".root").delegate(".del", "click", function(event){
		...
		if(type.indexOf("编辑") > -1){
			$.dialog.confirm(tip+"删除后无法恢复，请谨慎操作！！！", function(){
				...
				huoniao.operaJson("carFaguiType.php?dopost=del", "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() {
							location.reload();
			...
			ok: function(){
				...
				if(ids.length <= 0){
					...
					$.dialog.confirm("确定后，此分类下的子级和信息也将同时删除！<br />删除后无法恢复，请谨慎操作！！！", function(){
						...
						huoniao.showTip("loading", "正在删除，，请稍候...");
						huoniao.operaJson("carFaguiType.php?dopost=del", "id="+ids.join(","), function(data){
							if(data.state == 100){
...
function saveOpera(type){
	...
	huoniao.operaJson("carFaguiType.php?dopost=typeAjax", "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
