	...
	huoniao.operaJson("businessList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < businessList.length; i++){
				...
				list.push('  <td class="row15 left">'+img+'<span><a href="'+businessList[i].url+'" target="_blank">'+businessList[i].title+'</a><br />['+businessList[i].busiTypename+']<br />'+auth.join("")+'</span></td>');
				...
				list.push('  <td class="row15 left"'+(!businessList[i].state_time ? ' style="color:#ccc;" title="已过期"' : '')+'>'+businessList[i].pubdate+'<br>'+businessList[i].expired+'</td>');
				var state = "";
