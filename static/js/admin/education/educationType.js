$(function(){
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("educationType.php?dopost=updateType&id="+id+"&action="+action, "type=single&typename="+value, function(data){
				if(data.state == 100){
	...
	$(".root").delegate(".del", "click", function(event){
		...
		if(t.parent().parent().next("ul").html() != undefined && t.parent().parent().next("ul").html() != ""){
			...
			if(type.indexOf("编辑") > -1){
				huoniao.operaJson("educationType.php?dopost=del&action="+action, "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() {
							location.reload();
			...
			ok: function(){
				...
				if(ids.length <= 0){
					...
					$.dialog.confirm("确定后，此分类下的子级和帖子信息也将同时删除！<br />删除后无法恢复，请谨慎操作！！！", function(){
						...
						huoniao.operaJson("educationType.php?dopost=del&action="+action, "id="+ids.join(","), function(data){
							if(data.state == 100){
...
function saveOpera(type){
	...
	huoniao.operaJson("educationType.php?dopost=typeAjax", "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
					location.href = href;
