...
$(function(){
	...
	function checkGw(t, val, id){
		var flag = false;
		t.addClass("input-loading");
		huoniao.operaJson("../inc/json.php?action=checkUser_educationstore", "key="+val+"&id="+id, function(data){
			t.removeClass("input-loading");
			if(data == 200){
				t.siblings(".input-tips").removeClass().addClass("input-tips input-error").html('<s></s>此会员已授权管理其它教育公司，一个会员不可以管理多个教育公司！');
			}else{
				if(data) {
					for(var i = 0; i < data.length; i++){
				...
				if(flag){
					t.siblings(".input-tips").removeClass().addClass("input-tips input-ok").html('<s></s>如果填写了，则此会员可以管理教育公司');
				}else{
	...
	$("#mark").bind("click", function(){
		$.dialog({
			...
			content: 'url:'+adminPath+'../api/map/mark.php?mod=education&lnglat='+$("#lnglat").val()+"&city="+mapCity+"&address="+$("#address").val(),
			width: 800,
			height: 500,
			max: true,
			ok: function(){
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//异步提交
		huoniao.operaJson("educationstoreAdd.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "save"){
