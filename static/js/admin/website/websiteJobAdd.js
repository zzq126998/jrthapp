$(function(){
	...
	// 点击提示中的选项，填入文本框
	$('#editform .input-tips a, #editform .help-inline a').click(function(){
		var t = $(this);
		if(t.attr('href').indexOf('javascript') >= 0){
			t.closest('dd').children('input').val(t.text());
		}
	}).attr('title', '点击使用');
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//异步提交
		huoniao.operaJson("websiteJobAdd.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "Add"){
					$.dialog({
						fixed: true,
					...
					$.dialog({
						...
						ok: function(){
							try{
								console.log(id+'===');
								$("body",parent.document).find("#nav-websiteJob" + website).click();
								parent.reloadPage($("body",parent.document).find("#body-websiteJob"+website));
								$("body",parent.document).find("#nav-websiteJobEdit"+id+" s").click();
							}catch(e){
							}
						},
						cancel: false
