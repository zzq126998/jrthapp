$(function(){
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//异步提交
		huoniao.operaJson("websiteHonorAdd.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "Add"){
					$.dialog({
						fixed: true,
					...
					$.dialog({
						...
						ok: function(){
							try{
								$("body",parent.document).find("#nav-websiteHonor" + website).click();
								parent.reloadPage($("body",parent.document).find("#body-websiteHonor"+website));
								$("body",parent.document).find("#nav-websiteHonorEdit"+id+" s").click();
							}catch(e){
