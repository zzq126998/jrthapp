$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("websiteJob.php?dopost=del", "website="+website+"&id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("websiteJobAdd"+website, "website", "添加招聘职位", "website/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("websiteJobEdit"+id, "website", title, "website/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("websiteJob.php?dopost=getList&website="+website, data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, websiteJob = val.websiteJob;
		...
		if(val.state == "100"){
			...
			for(i; i < websiteJob.length; i++){
				list.push('<tr data-id="'+websiteJob[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');


				list.push('  <td class="row40 left">'+websiteJob[i].title+'</td>');
				list.push('  <td class="row37 left">'+websiteJob[i].pubdate+'</td>');
				list.push('  <td class="row20">');
				list.push('<a data-id="'+websiteJob[i].id+'" data-title="'+websiteJob[i].title+'" href="websiteJobAdd.php?website='+website+'&dopost=edit&id='+websiteJob[i].id+'" title="修改" class="edit">修改</a>');
