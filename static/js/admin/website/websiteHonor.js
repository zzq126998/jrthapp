$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("websiteHonor.php?dopost=del", "website="+website+"&id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("websiteHonorAdd"+website, "website", "添加新照片", "website/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("websiteHonorEdit"+id, "website", title, "website/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("websiteHonor.php?dopost=getList&website="+website, data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, websiteHonor = val.websiteHonor;
		...
		if(val.state == "100"){
			...
			for(i; i < websiteHonor.length; i++){
				list.push('<tr data-id="'+websiteHonor[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');


				list.push('  <td class="row40 left"><img src="'+cfg_attachment+websiteHonor[i].litpic+'" width="100" />&nbsp;&nbsp;'+websiteHonor[i].title+'</td>');
				list.push('  <td class="row37 left">'+websiteHonor[i].pubdate+'</td>');
				list.push('  <td class="row20">');
				list.push('<a data-id="'+websiteHonor[i].id+'" data-title="'+websiteHonor[i].title+'" href="websiteHonorAdd.php?website='+website+'&dopost=edit&id='+websiteHonor[i].id+'" title="修改" class="edit">修改</a>');
