	...
	huoniao.operaJson("memberList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < memberList.length; i++){
				...
				list.push('  <td class="row14 left"><img onerror="javascript:this.src=\'/static/images/default_user.jpg\';" src="'+cfg_attachment+memberList[i].photo+'&type=small" class="litpic" style="width:60px; height:60px;" /><span>'+memberList[i].username+'<br /><small>'+memberList[i].addrname+'</small></span></td>');
				var company = "";
				var certify = "";
