$(function(){
	...
	var defaultBtn = $("#reg, #cangelreg,#delBtn"),
		checkedBtn = $("#stateBtn"),
		init = {
			...
			,operaJson: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("tuanVoucherList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
...
function getList(){
	...
	huoniao.operaJson("tuanVoucherList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < tuanQuanList.length; i++){
				...
				list.push('  <td class="row25"><a href="javascript:;" data-id="'+tuanQuanList[i].uid+'" class="userinfo">'+tuanQuanList[i].username+'</a></td>');
				list.push('  <td class="row30"><a target="_blank" href="'+tuanQuanList[i].prourl+'">'+tuanQuanList[i].proname+'</a></td>');
				list.push('  <td class="row20">'+tuanQuanList[i].puctime+'</td>');
				/*var state = "";
				if(tuanQuanList[i].state == 1){
					state = "<span class='audit'>正在拼单</span>";
				}else if(tuanQuanList[i].state == 2){
					state = "<span class='gray'>已失效</span>";
				}else if(tuanQuanList[i].state == 3){
					state = "<span class='refuse'>拼单成功</span>";
				}else{
					state = "<span class='gray'>未付款</span>";
				}
				list.push('  <td class="row10">'+state+'</td>');*/
				list.push('  <td class="row20"><a href="javascript:;" title="删除" class="del">删除</a></td>');
			}
