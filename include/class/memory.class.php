...
class memory {
    ...
    public function init($config = array()) {
        ...
        $config = $config ? $config : $cfg_memory;

        if(!$config) return false;
        $this->config = $config;
        $this->prefix = empty($cfg_memory['prefix']) ? substr(md5($_SERVER['HTTP_HOST']), 0, 6).'_' : $cfg_memory['prefix'];
        unset($this->config['prefix']);

        foreach($this->config as $cache => $config) {
            if(!$test && $config['state'] != 1){
                continue;
            }
            $available = is_array($config) ? !empty($config['server']) : !empty($config);
            if($available && !is_object($this->memory)) {
                $class_name = 'memory_'.$cache;
                if(is_file(HUONIAOINC."/class/memory_redis.class.php")){
