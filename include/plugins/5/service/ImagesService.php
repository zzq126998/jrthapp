...
class ImagesService
{
    ...
    public function htmlImgs($html = '')
    {
        ...
        $img = $img[0];
        foreach ($img as $g) {
            preg_match_all('/<img(.*?)src=["|\']?(.*?)["|\']?\s(.*?)>/i', $g, $src);

            $src = empty($src) ? '' : $src[count($src) - 2];
            $src = $src[0];
    ...
    {
        $arr       = [];
        $save_path = HUONIAOROOT . '/uploads/plugins/' . date("Y") . '/' . date("m") . '/' . date("d") . '/' . 'reprint_' . $this->node . '/' . 'url_' . $this->urlId . '/';
...
//        }
        ...
        $newsPath = '//' . $_SERVER['HTTP_HOST'] . '/uploads/plugins/' . date("Y") . '/' . date("m") . '/' . date("d") . '/reprint_' . $this->node . '/url_' . $this->urlId . '/' . $filename;

        if(!file_exists(HUONIAOROOT . '/uploads/plugins/' . date("Y") . '/' . date("m") . '/' . date("d") . '/reprint_' . $this->node . '/url_' . $this->urlId . '/' . $filename)){
          $newsPath = $img_url;
        }
