...
if($userType == 3){
	//查询分类商家
	$sql = $dsql->SetQuery("SELECT `id` FROM `#@__infoshop` WHERE `cityid` in ($adminCityIds) AND `state` = 1");
...
if($dopost == "getDetail"){
	...
	if(count($results) > 0){
		...
		// $typeSql = $dsql->SetQuery("SELECT p.`title` FROM `#@__infoshop` p WHERE p.`id` = ". $results[0]["aid"]);
		// $typename = $dsql->getTypeName($typeSql);
		// $results[0]["storeTitle"] = $typename[0]['title'];

		$typeSql = $dsql->SetQuery("SELECT p.`company`, p.`nickname` FROM `#@__member` p WHERE p.`id` = ". $results[0]["userid"]);
		$typename = $dsql->getTypeName($typeSql);
...
//更新评论信息
	...
	//会员通知
	$sql = $dsql->SetQuery("SELECT l.`id`, l.`jointime` pubdate, l.`uid`, c.`userid`, c.`ischeck`, c.`dtime` FROM `#@__".$action."_comment` c LEFT JOIN `#@__infoshop` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	$ret = $dsql->dsqlOper($sql, "results");
	if($ret){
		$aid     = $ret[0]['id'];
		// $title   = $ret[0]['title'];
...
//更新评论状态
	...
	foreach($each as $val){
		...
		//会员通知
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`jointime` as pubdate, l.`uid` , c.`userid`, c.`ischeck`, c.`dtime` FROM `#@__".$action."_comment` c LEFT JOIN `#@__infoshop` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val.$where);
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			$aid     = $ret[0]['id'];
			// $title   = $ret[0]['title'];
			...
			$typeSql = $dsql->SetQuery("SELECT p.`company`, p.`nickname` FROM `#@__member` p WHERE p.`id` = ". $ret[0]["userid"]);
			$typename= $dsql->getTypeName($typeSql);
			$title   = $typename[0]['company'] ? $typename[0]['company'] : $typename[0]['nickname'];
	...
	if(!empty($error)){
		...
		adminLog("更新分类商家评论状态", $id."=>".$arcrank);
		echo '{"state": 100, "info": '.json_encode("修改成功！").'}';
	}
	die;
...
//获取评论列表
	...
	$where = " AND `type` = 'info-business'";

	if ($adminCity){
		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__infoshop` WHERE `cityid` = '$adminCity' AND `state` = 1");
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			foreach ($ret as $key => $value) {
				array_push($ids, $value['id']);
	...
	if($sKeyword != ""){
		...
		//按信息标题搜索
			...
			$archives = $dsql->SetQuery("SELECT l.`id` FROM `#@__infoshop` l WHERE l.`title` like '%$sKeyword%'");
			$results = $dsql->dsqlOper($archives, "results");
			if(count($results) > 0){
				$list = array();
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			// $typeSql = $dsql->SetQuery("SELECT `title` FROM `#@__infoshop` WHERE `id` = ". $value["aid"]);
			// $typename = $dsql->getTypeName($typeSql);
			// $list[$key]["storeTitle"] = $typename[0]['title'];

			$typeSql = $dsql->SetQuery("SELECT p.`company`, p.`nickname` FROM `#@__member` p WHERE p.`id` = ". $value["aid"]);
			$typename= $dsql->getTypeName($typeSql);
