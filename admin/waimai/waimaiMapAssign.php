...
if(file_exists($tpl."/".$templates)){
	...
	//配送员信息
    $courier = array();
    $sql = $dsql->SetQuery("SELECT c.*, (SELECT count(`id`) FROM `#@__waimai_order` WHERE (`state` = 4 OR `state` = 5) AND `peisongid` = c.`id`) as count FROM `#@__waimai_courier` c WHERE c.`state` = 1 AND c.`quit` = 0 ORDER BY count ASC");
