 ...
 * 管理店铺
 *
 * @version        $Id: educationstoreList.php 2019-07-10 上午11:28:16 $
...
checkPurview("educationstoreList");
$dsql  = new dsql($dbo);
...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `cityid`, `title`, `userid`, `tel`, `address`, `state`,  `weight`, `pubdate`, `flag` FROM `#@__".$tab."` WHERE 1 = 1".$where);
...
//删除
	...
	if(!testPurview("educationstoreDel")){
	...
	if($id != ""){
		...
		foreach($each as $val){
			...
			$archives = $dsql->SetQuery("SELECT `pics` FROM `#@__education_store` WHERE `id` = ".$val);
			$results = $dsql->dsqlOper($archives, "results");
		...
		if(!empty($error)){
			...
			checkeducationStoreCache($id);
			adminLog("删除教育店铺信息", join(", ", $title));
...
// 检查缓存
function checkeducationStoreCache($id){
