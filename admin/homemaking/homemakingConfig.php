 ...
 * 家政服务配置
 *
 * @version        $Id: homemakingConfig.php 2019-04-01 上午10:12:36 $
...
checkPurview("homemakingConfig");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__) . "/../templates/homemaking";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "homemakingConfig.html";

$action = $action == "" ? "homemaking" : $action;
...
if ($type != "") {
    ...
    if ($type == "site") {
        ...
        $customhomemakingCheck = (int)$homemakingCheck;
        $custompersonalCheck = (int)$personalCheck;
        $customnannyCheck = (int)$nannyCheck;
        ...
        $custom_homemaking_atlasMax = (int)$homemaking_atlasMax;

        $customshomemakingTag     = $homemakingTag;
        $customshomemakingFlag    = $homemakingFlag;

        $customrefundReason      = $refundReason;
        $customafterSalesType    = $afterSalesType;
    ...
    $customInc .= "\$customhomemakingCheck = '" . $customhomemakingCheck . "';\r\n";
    $customInc .= "\$custompersonalCheck = '" . $custompersonalCheck . "';\r\n";
    $customInc .= "\$customnannyCheck = '" . $customnannyCheck . "';\r\n";
    $customInc .= "\$customshomemakingTag = '".$customshomemakingTag."';\r\n";
    $customInc .= "\$customshomemakingFlag = '".$customshomemakingFlag."';\r\n";
    $customInc .= "\$customrefundReason = '".$customrefundReason."';\r\n";
    $customInc .= "\$customafterSalesType = '".$customafterSalesType."';\r\n";
    
    

    $customInc .= "\$custom_store_atlasMax = " . (int)$custom_store_atlasMax . ";\r\n";
    $customInc .= "\$custom_homemaking_atlasMax = " . (int)$custom_homemaking_atlasMax . ";\r\n";
...
if (file_exists($tpl . "/" . $templates)) {
    ...
    $jsFile = array(
        ...
        'publicUpload.js',
        'admin/homemaking/homemakingConfig.js'
    );
    ...
    if(!isset($customshomemakingTag)){
		$customshomemakingTag = '售后保障|随心退';
	}
    $huoniaoTag->assign('homemakingTag', $customshomemakingTag);
    
    if(!isset($customshomemakingFlag)){
		$customshomemakingFlag = '3小时上门|今日可到';
	}
    $huoniaoTag->assign('homemakingFlag', $customshomemakingFlag);

    if(!isset($customrefundReason)){
		$customrefundReason = '决定选择其他商家|计划时间有变|买错了/买多了|协商一致退款';
	}
    $huoniaoTag->assign('refundReason', $customrefundReason);

    if(!isset($customafterSalesType)){
		$customafterSalesType = '服务不满意想要退款|商家未上门服务|其他原因';
	}
    $huoniaoTag->assign('afterSalesType', $customafterSalesType);

    //基本设置
    $huoniaoTag->assign('channelname', $customChannelName);
    $huoniaoTag->assign('homemakingCheckNum', array('0', '1'));
    $huoniaoTag->assign('homemakingCheckNames', array('需要审核', '不需要审核'));
    $huoniaoTag->assign('homemakingCheckChecked', (int)$customhomemakingCheck);

    //服务人员
    $huoniaoTag->assign('personalCheckNum', array('0', '1'));
    $huoniaoTag->assign('personalCheckNames', array('需要审核', '不需要审核'));
    $huoniaoTag->assign('personalCheckChecked', (int)$custompersonalCheck);

    //保姆/月嫂
    $huoniaoTag->assign('nannyCheckNum', array('0', '1'));
    $huoniaoTag->assign('nannyCheckNames', array('需要审核', '不需要审核'));
    $huoniaoTag->assign('nannyCheckChecked', (int)$customnannyCheck);
    ...
    $huoniaoTag->assign('homemaking_atlasMax', $custom_homemaking_atlasMax);
