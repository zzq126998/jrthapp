 ...
 * 经销商自定义认证属性
 *
 * @version        $Id: carAuthAttr.php 2019-03-14 上午11:57:36 $
 * @package        HuoNiao.car
...
checkPurview("carAuthAttr");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/car";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "carAuthAttr.html";
	...
	if($id != ""){
		...
		if(!empty($results)){
			...
			if($results != "ok"){
				...
				$title = $type == "jc" ? "简称" : "";
				adminLog("修改经销商自定义认证属性".$title, $value);
...
//删除
	...
	if($id != ""){
		...
		adminLog("删除经销商自定义认证属性", join(",", $idsArr));
		...
		//其它为数据库已存在
			...
			if(!empty($results)){
				...
				if($results[0]["jc"] != $jc){
					...
					adminLog("修改经销商自定义认证属性简称", $name."=>".$jc);
				}
				...
				if($results[0]["weight"] != $i){
					...
					adminLog("修改经销商自定义认证属性排序", $name."=>".$i);
				}
