 ...
 * 汽车频道配置
 *
 * @version        $Id: carConfig.php 2019-03-14 上午10:12:36 $
 * @package        HuoNiao.car
...
if ($type != "") {
    ...
    if ($type == "site") {
        ...
        $custom_car_atlasMax = (int)$car_atlasMax;
        $customCarTag     = $carTag;
        $customsecuritysettingTag     = $securitysettingTag;
        $custominternalsettingTag     = $internalsettingTag;
        $customexternalsettingTag     = $externalsettingTag;
    ...
    $customInc .= "\$customcarCheck = '" . $customcarCheck . "';\r\n";
    $customInc .= "\$customCarTag = '".$customCarTag."';\r\n";
    $customInc .= "\$customsecuritysettingTag = '".$customsecuritysettingTag."';\r\n";
    $customInc .= "\$custominternalsettingTag = '".$custominternalsettingTag."';\r\n";
    $customInc .= "\$customexternalsettingTag = '".$customexternalsettingTag."';\r\n";

    $customInc .= "\$custom_store_atlasMax = " . (int)$custom_store_atlasMax . ";\r\n";
    $customInc .= "\$custom_car_atlasMax = " . (int)$custom_car_atlasMax . ";\r\n";
...
if (file_exists($tpl . "/" . $templates)) {
    ...
    if(!isset($customCarTag)){
		$customCarTag = '车辆置换|价格评估|分期贷款|包办过户|代办验车|代办上牌|代办转籍|车辆寄售|延长质保';
	}
    $huoniaoTag->assign('carTag', $customCarTag);
    
    if(!isset($customsecuritysettingTag)){
    ...
    $huoniaoTag->assign('securitysettingTag', $customsecuritysettingTag);
    
    if(!isset($customexternalsettingTag)){
		$customexternalsettingTag = '1-电动天窗|2-全景天窗|3-电动吸合门|4-感应后备箱|5-感应雨刷|6-后雨刷|7-前电动车窗|8-后电动车窗|9-后视镜电动调节|10-后视镜加热';
	}
    $huoniaoTag->assign('externalsettingTag', $customexternalsettingTag);
    
    if(!isset($custominternalsettingTag)){
		$custominternalsettingTag = '1-多功能方向盘|2-定速巡航|3-空调|4-自动空调|5-GPS导航|6-倒车雷达|7-倒车影像系统|8-真皮座椅|9-前排座椅加热|10-后排座椅加热 ';
	}
	$huoniaoTag->assign('internalsettingTag', $custominternalsettingTag);

    //基本设置
    $huoniaoTag->assign('channelname', $customChannelName);
    $huoniaoTag->assign('carCheckNum', array('0', '1'));
    $huoniaoTag->assign('carCheckNames', array('需要审核', '不需要审核'));
    $huoniaoTag->assign('carCheckChecked', (int)$customcarCheck);
    ...
    $huoniaoTag->assign('car_atlasMax', $custom_car_atlasMax);
