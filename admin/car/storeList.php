 ...
 * 管理经销商
 *
 * @version        $Id: storeList.php 2019-03-14 上午11:28:16 $
 * @package        HuoNiao.car
...
//删除
	...
	if($id != ""){
		...
		foreach($each as $val){
			...
			$archives = $dsql->SetQuery("SELECT `logo`, `license`, `pics`, `note` FROM `#@__car_store` WHERE `id` = ".$val);
			$results = $dsql->dsqlOper($archives, "results");

			//删除缩略图
			delPicFile($results[0]['logo'], "dellogo", "car");
			delPicFile($results[0]['license'], "delThumb", "car");
		...
		if(!empty($error)){
			...
			checkCarStoreCache($id);
			adminLog("删除经销商信息", join(", ", $title));
...
// 检查缓存
function checkCarStoreCache($id){
