 ...
 * 管理品牌类别
 *
 * @version        $Id: carBrandType.php 2019-03-25 下午22:37:05 $
 * @package        HuoNiao.car
...
checkPurview("carBrandType");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/car";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "carBrandType.html";

$action = "car_brandtype";
...
//修改分类
	...
	if(!empty($results)){
		...
		if($type == "single"){
			...
			$hot = $hot ? $hot : 0;
			//保存到主表
			$archives = $dsql->SetQuery("UPDATE `#@__".$action."` SET `hot` = '$hot', `parentid` = '$parentid', `typename` = '$typename', `pinyin` = '$pinyin', `py` = '$py' WHERE `id` = ".$id);
			$results = $dsql->dsqlOper($archives, "update");
		}
...
//删除分类
	...
	foreach ($idexp as $k => $id) {
		...
		foreach ($idsArr as $kk => $id) {
			...
			if($res){
				delPicFile($res[0]['icon'], "delAdv", "car");
			}
		}
