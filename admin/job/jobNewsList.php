...
//新增
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`cityid`, `title`, `typeid`, `litpic`, `click`, `weight`, `source`, `sourceUrl`, `writer`, `keyword`, `description`, `body`, `arcrank`, `pubdate`) VALUES ('$cityid', '$title', '$typeid', '$litpic', '$click', '$weight', '$source', '$sourceUrl', '$writer', '$keyword', '$description', '$body', '$arcrank', '$pubdate')");
		...
		if(is_numeric($aid)){
			...
			if($arcrank == 0){
				// 清除缓存
				clearCache("job_news_total", "key");
				clearCache("job_news_list", "key");
			}
...
//修改
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `cityid` = '$cityid', `title` = '$title', `typeid` = $typeid, `litpic` = '$litpic', `click` = '$click', `weight` = '$weight', `source` = '$source', `sourceUrl` = '$sourceUrl', `writer` = '$writer', `keyword` = '$keyword', `description` = '$description', `body` = '$body', `arcrank` = '$arcrank' WHERE `id` = ".$id);
		...
		if($return == "ok"){
			...
			// 清除缓存
			clearCache("job_news_detail", $id);
			if(($arcrank != 1 && $arcrank_ == 1)|| ($arcrank == 1 && $arcrank_ != 1)){
				checkCache("job_news_list", $id);
				clearCache("job_news_total", "key");
			}
