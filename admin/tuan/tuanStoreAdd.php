...
if($dopost == "save" && $submit == "提交"){
	...
	//保存到表
    $archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`uid`, `stype`, `addrid`, `address`, `circle`, `subway`, `lnglat`, `tel`, `openStart`, `openEnd`, `note`, `body`, `jointime`, `click`, `weight`, `state`, `cityid`, `istop`, `lng`, `lat`, `video`, `pics`, `phone`, `shop_openid`, `qq`, `wechatcode`) VALUES ('$uid', '$stype', '$addrid', '$address', '$circle', '$subway', '$lnglat', '$tel', '$openStart', '$openEnd', '$note', '$body', '".GetMkTime(time())."', '$click', '$weight', '$state', '$cityid', '$istop', '$lng', '$lat', '$video', '$imglist', '$phone', '$shop_openid', '$qq', '$wechatcode')");
	...
	if($submit == "提交"){
		...
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `uid` = '$uid', `stype` = '$stype', `addrid` = '$addrid', `address` = '$address', `circle` = '$circle', `subway` = '$subway', `lnglat` = '$lnglat', `tel` = '$tel', `openStart` = '$openStart', `openEnd` = '$openEnd', `note` = '$note', `body` = '$body', `click` = '$click', `weight` = '$weight', `state` = '$state', `cityid` = '$cityid', `istop`='$istop', `lng` = '$lng', `lat`='$lat', `video`='$video', `pics`='$imglist', `phone`='$phone', `shop_openid` = '$shop_openid', `qq` = '$qq', `wechatcode` = '$wechatcode' WHERE `id` = ".$id);
