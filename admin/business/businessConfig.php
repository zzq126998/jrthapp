...
if($type != ""){
	...
	//入驻费用
		...
		$custom_trialName = $trialName ? $trialName : "体验版";

		$trialCostArr = array();
		if($trialCost){
			for ($i = 0; $i < count($trialCost['time']); $i++) {
				$time = (int)$trialCost['time'][$i];
				$type = $trialCost['type'][$i];
				$price = (float)$trialCost['price'][$i];
				$point = (int)$trialCost['point'][$i];
				$label = $trialCost['label'][$i];
				if($time){
					array_push($trialCostArr, array(
						'time' => $time,
						'type' => $type,
						'price' => $price,
						'point' => $point,
		...
		$custom_trialCost = serialize($trialCostArr);

		$custom_enterpriseState = (int)$enterpriseState;
		$custom_enterpriseAutoAudit = (int)$enterpriseAutoAudit;
		$custom_enterpriseName = $enterpriseName ? $enterpriseName : "企业版";

		$enterpriseCostArr = array();
		if($enterpriseCost){
			for ($i = 0; $i < count($enterpriseCost['time']); $i++) {
				$time = (int)$enterpriseCost['time'][$i];
				$type = $enterpriseCost['type'][$i];
				$price = (float)$enterpriseCost['price'][$i];
				$point = (int)$enterpriseCost['point'][$i];
				$label = $enterpriseCost['label'][$i];
				if($time){
					array_push($enterpriseCostArr, array(
						'time' => $time,
						'type' => $type,
						'price' => $price,
						'point' => $point,
		...
		$custom_enterpriseCost = serialize($enterpriseCostArr);

		$custom_joinAuth = serialize($joinAuth);

		adminLog("修改商家设置", "入驻费用");
	...
	//入驻费用
	// $customInc .= "\$customCost = '".$customCost."';\r\n";
	$customInc .= "\$custom_trialState = ".(int)$custom_trialState.";\r\n";
	$customInc .= "\$custom_trialAutoAudit = ".(int)$custom_trialAutoAudit.";\r\n";
	$customInc .= "\$custom_trialName = '".$custom_trialName."';\r\n";
	$customInc .= "\$custom_trialCost = '".$custom_trialCost."';\r\n";
	$customInc .= "\$custom_enterpriseState = ".(int)$custom_enterpriseState.";\r\n";
	$customInc .= "\$custom_enterpriseAutoAudit = ".(int)$custom_enterpriseAutoAudit.";\r\n";
...
if(file_exists($tpl."/".$templates)){
	...
	//打印机配置
	$huoniaoTag->assign('printPlatList', array(0 => '易联云'));
