 ...
 * 管理全景评论
 *
 * @version        $Id: quanjingCommon.php 2019-5-6 上午10:33:36 $
 * @package        HuoNiao.quanjing
 * @copyright      Copyright (c) 2013 - 2019, HuoNiao, Inc.
 * @link           https://www.ihuoniao.cn/
 */
define('HUONIAOADMIN', "..");
require_once(dirname(__FILE__)."/../inc/config.inc.php");
checkPurview("quanjingCommon");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/quanjing";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "quanjingCommon.html";
...
if($userType == 3){
	//查询信息
	$sql = $dsql->SetQuery("SELECT `id` FROM `#@__quanjinglist` WHERE `cityid` in ($adminCityIds) AND `arcrank` = 1");
...
if($dopost == "getDetail"){
	...
	if(count($results) > 0){
		...
		$typeSql = $dsql->SetQuery("SELECT p.`title` FROM `#@__quanjinglist` p WHERE p.`id` = ". $results[0]["aid"]);
...
//更新评论信息
	...
	//会员通知
	$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`admin` uid, c.`userid`, c.`ischeck`, c.`dtime` FROM `#@__".$action."_comment` c LEFT JOIN `#@__quanjinglist` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	$ret = $dsql->dsqlOper($sql, "results");
	if($ret){
		$aid     = $ret[0]['id'];
		$title   = $ret[0]['title'];
...
//更新评论状态
	...
	foreach($each as $val){
		...
		//会员通知
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`admin` uid, c.`userid`, c.`ischeck`, c.`dtime` FROM `#@__".$action."_comment` c LEFT JOIN `#@__quanjinglist` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val.$where);
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			$aid     = $ret[0]['id'];
			$title   = $ret[0]['title'];
...
//获取评论列表
	...
	$where = " AND `type` = 'quanjing-detail'";

	if ($adminCity){
		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__quanjinglist` WHERE `cityid` = '$adminCity' AND `arcrank` = 1");
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			foreach ($ret as $key => $value) {
				array_push($ids, $value['id']);
	...
	if($sKeyword != ""){
		...
		//按信息标题搜索
			...
			$archives = $dsql->SetQuery("SELECT l.`id` FROM `#@__quanjinglist` l WHERE l.`title` like '%$sKeyword%'");
			$results = $dsql->dsqlOper($archives, "results");
			if(count($results) > 0){
				$list = array();
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$typeSql  = $dsql->SetQuery("SELECT `title` FROM `#@__quanjinglist` WHERE `id` = ". $value["aid"]);
			$typename = $dsql->getTypeName($typeSql);
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/quanjing/quanjingCommon.js'
	);
