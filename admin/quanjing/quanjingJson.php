 ...
 * 全景模块JSON
 *
 * @version        $Id: quanjingJson.php 2018-6-6 下午16:48:10 $
		...
		foreach($each as $val){
			...
			if($file){
				if(strpos($file, "http") === false){
					$file_ = HUONIAOROOT.$file;
					if(file_exists($file_)) {
						// 遍历解压包
						$filePath = dirname($file_);
			       		removeDir($filePath);
					}else{
						delPicFile($file, "delFlash", $dopost);
					}
				}
			}
