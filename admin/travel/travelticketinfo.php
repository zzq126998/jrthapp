 ...
 * 管理门票信息
 *
 * @version        $Id: travelticketinfo.php 2019-5-20 上午00:07:12 $
...
$templates = "travelticketinfo.html";
...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `title`, `sale`, `ticketid`, `price`, `pubdate`, `weight` FROM `#@__".$tab."` WHERE `ticketid` = ".$id.$where);
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$videoSql    = $dsql->SetQuery("SELECT `id`, `title` FROM `#@__travel_ticket` WHERE `id` = ". $value['ticketid']);
			$videoResult = $dsql->getTypeName($videoSql);
			$list[$key]["hostname"] = $videoResult[0]["title"];
		}

		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "travelticket": '.json_encode($list).'}';
		}else{
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/travel/travelticketinfo.js'
	);
	...
	if($action == "travelticket"){
		//楼盘信息
		$videoSql = $dsql->SetQuery("SELECT `id`, `title` FROM `#@__travel_ticket` WHERE `id` = ". $id);
		$videoResult = $dsql->getTypeName($videoSql);
		if(!$videoResult)die('景点门票不存在！');
