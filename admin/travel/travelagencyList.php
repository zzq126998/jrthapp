 ...
 * 管理周边游
 *
 * @version        $Id: travelagencyList.php 2019-04-01 上午11:28:16 $
...
$templates = "travelagencyList.html";
...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `title`, `company`, `state`, `weight`, `pubdate`, `typeid`, (SELECT avg(c.`price`) FROM `#@__travel_ticketinfo` c WHERE c.`ticketid` = l.`id` AND c.`typeid` = '1' ) AS price FROM `#@__".$tab."` l WHERE 1 = 1".$where);
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$list[$key]["price"]   = $value["price"] ? round($value["price"], 2) : 0;
			...
			$travel = new travel();
			if($value['typeid']!=''){
				$typename = $travel->gettypename("travelagency_type", $value['typeid']);
			}
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.', "totalGray": '.$totalGray.', "totalAudit": '.$totalAudit.', "totalRefuse": '.$totalRefuse.'}, "agencyList": '.json_encode($list).'}';
		}else{
...
//删除
	...
	if($id != ""){
		...
		foreach($each as $val){
			...
			$archives = $dsql->SetQuery("SELECT `pics`, `imglist`, `video`, `note` FROM `#@__".$tab."` WHERE `id` = ".$val);
			$results = $dsql->dsqlOper($archives, "results");
			//删除缩略图
			delPicFile($results[0]['pics'], "delAtlas", "travel");
			delPicFile($results[0]['imglist'], "delAtlas", "travel");
			...
			$sql = $dsql->SetQuery("DELETE FROM `#@__travel_ticketinfo` WHERE `typeid` = 1 AND  `ticketid` = ".$val);
		...
		if(!empty($error)){
			...
			checktravelagencyCache($id);
			adminLog("删除周边游信息", join(", ", $title));
...
// 检查缓存
function checktravelagencyCache($id){
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/travel/travelagencyList.js'
	);
