 ...
 * 管理旅游租车
 *
 * @version        $Id: travelrentcarList.php 2019-04-01 上午11:28:16 $
...
checkPurview("travelrentcarList");
$dsql  = new dsql($dbo);
$tpl   = dirname(__FILE__)."/../templates/travel";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "travelrentcarList.html";
...
if($dopost == "getList"){
	...
	if($sType){
		if($dsql->getTypeList($sType, "travel_rentcartype")){
			$lower = arr_foreach($dsql->getTypeList($sType, "travel_rentcartype"));
	...
	$archives = $dsql->SetQuery("SELECT `id`, `title`, `company`, `state`, `weight`, `pubdate`, `typeid`, `price` FROM `#@__".$tab."` WHERE 1 = 1".$where);
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$sql = $dsql->SetQuery("SELECT `typename` FROM `#@__travel_rentcartype` WHERE `id` = ". $value['typeid']);
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.', "totalGray": '.$totalGray.', "totalAudit": '.$totalAudit.', "totalRefuse": '.$totalRefuse.'}, "rentcarList": '.json_encode($list).'}';
		}else{
...
//删除
	...
	if(!testPurview("travelrentcarDel")){
	...
	if($id != ""){
		...
		if(!empty($error)){
			...
			checktravelrentcarCache($id);
			adminLog("删除旅游租车信息", join(", ", $title));
...
// 检查缓存
function checktravelrentcarCache($id){
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/travel/travelrentcarList.js'
	);
