 ...
 * 管理旅游材料分类
 *
 * @version        $Id: travelItem.php 2019-5-21 下午22:29:11 $
...
checkPurview("travelItem");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/travel";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "travelItem.html";
...
//修改分类
	...
	if(!empty($results)){
		...
		if($type == "single"){
			...
			//保存到主表
			$archives = $dsql->SetQuery("UPDATE `#@__".$action."` SET `typename` = '$typename', `typeid` = '$typeid', `description` = '$description' WHERE `id` = ".$id);
			$results = $dsql->dsqlOper($archives, "update");
		}
		...
		if($results != "ok"){
			...
			checkCache("travel_item_list", $id);
			clearCache("travel_item_detail", $id);
			clearCache("travel_item_total", 'key');

			adminLog("修改旅游材料分类", $typename);
