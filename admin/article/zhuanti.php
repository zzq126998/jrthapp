...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id` FROM `#@__".$db."` WHERE `parentid` = 0");
	...
	$archives = $dsql->SetQuery("SELECT * FROM `#@__".$db."` WHERE `parentid` = 0".$where);
...
//删除专题
	...
	if($id != ""){
		...
		foreach ($idexp as $k => $id) {
			$childArr = $dsql->getTypeList($id, "article_zhuanti", 1);
		...
		foreach ($idsArr as $kk => $id) {
			...
			if($results != "ok"){
				...
				$sql = $dsql->SetQuery("DELETE FROM `#@__article_zhuantilist` WHERE `typeid` = ".$id);
				$dsql->dsqlOper($sql, "update");
				$sql = $dsql->SetQuery("UPDATE `#@__articlelist_all` SET `zhuanti` = 0 WHERE `zhuanti` = $id");
				$dsql->dsqlOper($sql, "update");
			}

		}

		adminLog("删除新闻专题", join(",", $idsArr));
