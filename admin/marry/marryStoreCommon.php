...
if($userType == 3){
	//查询信息
	if($typeid == 0){
		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__marry_store` WHERE `cityid` in ($adminCityIds) AND `state` = 1");
	}elseif($typeid == 1){
		$comSql = $dsql->SetQuery("SELECT `id` FROM `#@__marry_store` WHERE `cityid` in ($adminCityIds) AND `state` = 1");
		...
		if($comResult) {
			...
			$sql = $dsql->SetQuery("SELECT `id` FROM `#@__marry_weddingcar` WHERE `state` = 1 $where1");
		}else{
...
if($dopost == "getDetail"){
	...
	if(count($results) > 0){
		...
		if($typeid == 0){
			$typeSql = $dsql->SetQuery("SELECT p.`title` FROM `#@__marry_store` p WHERE p.`id` = ". $results[0]["aid"]);
		}elseif($typeid == 1){
			$typeSql = $dsql->SetQuery("SELECT p.`title` FROM `#@__marry_weddingcar` p WHERE p.`id` = ". $results[0]["aid"]);
		}
...
//更新评论信息
	...
	//会员通知
	if($typeid == 0){
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime` FROM `#@__".$action."_comment` c LEFT JOIN `#@__marry_store` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	}elseif($typeid == 1){
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`company`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime` FROM `#@__".$action."_comment` c LEFT JOIN `#@__marry_weddingcar` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	}
	$ret = $dsql->dsqlOper($sql, "results");
	if($ret){
		$aid     = $ret[0]['id'];
		...
		if($typeid == 1){
			$sql = $dsql->SetQuery("SELECT `userid` FROM `#@__marry_store` WHERE `id` = " . $ret[0]['company']);
...
//更新评论状态
	...
	foreach($each as $val){
		...
		//会员通知
		if($typeid == 0){
			$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime` FROM `#@__".$action."_comment` c LEFT JOIN `#@__marry_store` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val.$where);
		}elseif($typeid == 1){
			$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`company`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime` FROM `#@__".$action."_comment` c LEFT JOIN `#@__marry_weddingcar` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val . $where);
		}
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			$aid     = $ret[0]['id'];
...
//获取评论列表
	...
	if($typeid == 0){
		$where = " AND `type` = 'marry-store'";
	}elseif($typeid == 1){
		$where = " AND `type` = 'marry-rental'";
	}

	if ($adminCity){
        if($typeid == 0){
			$sql = $dsql->SetQuery("SELECT `id` FROM `#@__marry_store` WHERE `cityid` = '$adminCity' AND `state` = 1");
		}elseif($typeid == 1){
			$comSql = $dsql->SetQuery("SELECT `id` FROM `#@__marry_store` WHERE `cityid` = '$adminCity' AND `state` = 1");
			$comResult = $dsql->dsqlOper($comSql, "results");
			if($comResult) {
				$comid = array();
				foreach ($comResult as $key => $com) {
				...
				$sql = $dsql->SetQuery("SELECT `id` FROM `#@__marry_weddingcar` WHERE `state` = 1 $where2");
			}else{
				echo '{"state": 101, "info": '.json_encode("暂无相关信息").', "pageInfo": {"totalPage": 0, "totalCount": 0, "totalGray": 0, "totalAudit": 0, "totalRefuse": 0}}';die;
			}
		}
	...
	if($sKeyword != ""){
		...
		//按信息标题搜索
			...
			if($typeid == 0){
				$archives = $dsql->SetQuery("SELECT l.`id` FROM `#@__marry_store` l WHERE l.`title` like '%$sKeyword%'");
				$results = $dsql->dsqlOper($archives, "results");
				if(count($results) > 0){
					$list = array();
				...
				$archives = $dsql->SetQuery("SELECT l.`id` FROM `#@__marry_weddingcar` l WHERE l.`title` like '%$sKeyword%'");
				$results = $dsql->dsqlOper($archives, "results");
				if(count($results) > 0){
					$list = array();
					foreach ($results as $key=>$value) {
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			if($typeid == 0){
				$typeSql = $dsql->SetQuery("SELECT `title` FROM `#@__marry_store` WHERE `id` = ". $value["aid"]);
			}elseif($typeid == 1){
				$typeSql = $dsql->SetQuery("SELECT `title` FROM `#@__marry_weddingcar` WHERE `id` = ". $value["aid"]);
			}
			$typename = $dsql->getTypeName($typeSql);
