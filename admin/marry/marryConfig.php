 ...
 * 婚嫁频道配置
 *
 * @version        $Id: marryConfig.php 2019-04-01 上午10:12:36 $
...
if ($type != "") {
    ...
    if ($type == "site") {
        ...
        $custommarryhotelfieldCheck = (int)$marryhotelfieldCheck;
        $custommarryhotelmenuCheck = (int)$marryhotelmenuCheck;
        $custommarryhostCheck = (int)$marryhostCheck;
        $customweddingcarCheck = (int)$weddingcarCheck;
        $custommarryplancaseCheck = (int)$marryplancaseCheck;
        $custommarryplanmealCheck = (int)$marryplanmealCheck;
        ...
        $custom_marryhotelfield_atlasMax = (int)$marryhotelfield_atlasMax;
        $custom_marryweddingcar_atlasMax = (int)$marryweddingcar_atlasMax;
        $custom_marryplancase_atlasMax = (int)$marryplancase_atlasMax;
        $custom_marryplanmeal_atlasMax = (int)$marryplanmeal_atlasMax;

        $custommarryFlag          = $marryFlag;
        $custommarryTag          = $marryTag;
    ...
    $customInc .= "\$custommarryhotelfieldCheck = '" . $custommarryhotelfieldCheck . "';\r\n";
    $customInc .= "\$custommarryhotelmenuCheck = '" . $custommarryhotelmenuCheck . "';\r\n";
    $customInc .= "\$custommarryhostCheck = '" . $custommarryhostCheck . "';\r\n";
    $customInc .= "\$customweddingcarCheck = '" . $customweddingcarCheck . "';\r\n";
    $customInc .= "\$custommarryplancaseCheck = '" . $custommarryplancaseCheck . "';\r\n";
    $customInc .= "\$custommarryplanmealCheck = '" . $custommarryplanmealCheck . "';\r\n";
    $customInc .= "\$customCommentCheck = ".(int)$customCommentCheck.";\r\n";

    $customInc .= "\$custommarryFlag = '".$custommarryFlag."';\r\n";
    $customInc .= "\$custommarryTag = '".$custommarryTag."';\r\n";

    $customInc .= "\$custom_store_atlasMax = " . (int)$custom_store_atlasMax . ";\r\n";
    $customInc .= "\$custom_marryhotelfield_atlasMax = " . (int)$custom_marryhotelfield_atlasMax . ";\r\n";
    $customInc .= "\$custom_marryweddingcar_atlasMax = " . (int)$custom_marryweddingcar_atlasMax . ";\r\n";
    $customInc .= "\$custom_marryplancase_atlasMax = " . (int)$custom_marryplancase_atlasMax . ";\r\n";
    $customInc .= "\$custom_marryplanmeal_atlasMax = " . (int)$custom_marryplanmeal_atlasMax . ";\r\n";
...
if (file_exists($tpl . "/" . $templates)) {
    ...
    if(!isset($custommarryFlag)){
		$custommarryFlag = '地铁沿线|独立门面|层高高';
	}
    $huoniaoTag->assign('marryFlag', $custommarryFlag);

    if(!isset($custommarryTag)){
		$custommarryTag = '本季热卖|超值特价|人气推荐';
	}
    $huoniaoTag->assign('marryTag', $custommarryTag);

    //婚宴场地
    $huoniaoTag->assign('channelname', $customChannelName);
    $huoniaoTag->assign('marryhotelfieldCheckNum', array('0', '1'));
    $huoniaoTag->assign('marryhotelfieldCheckNames', array('需要审核', '不需要审核'));
    $huoniaoTag->assign('marryhotelfieldCheckChecked', (int)$custommarryhotelfieldCheck);

    //婚宴菜单
    $huoniaoTag->assign('marryhotelmenuCheckNum', array('0', '1'));
    $huoniaoTag->assign('marryhotelmenuCheckNames', array('需要审核', '不需要审核'));
    $huoniaoTag->assign('marryhotelmenuCheckChecked', (int)$custommarryhotelmenuCheck);

    //主持人
    $huoniaoTag->assign('marryhostCheckNum', array('0', '1'));
    $huoniaoTag->assign('marryhostCheckNames', array('需要审核', '不需要审核'));
    $huoniaoTag->assign('marryhostCheckChecked', (int)$custommarryhostCheck);

    //婚车
    $huoniaoTag->assign('weddingcarCheckNum', array('0', '1'));
    $huoniaoTag->assign('weddingcarCheckNames', array('需要审核', '不需要审核'));
    $huoniaoTag->assign('weddingcarCheckChecked', (int)$customweddingcarCheck);

    //商家案例
    $huoniaoTag->assign('marryplancaseCheckNum', array('0', '1'));
    $huoniaoTag->assign('marryplancaseCheckNames', array('需要审核', '不需要审核'));
    $huoniaoTag->assign('marryplancaseCheckChecked', (int)$custommarryplancaseCheck);

    //婚嫁套餐
    $huoniaoTag->assign('marryplanmealCheckNum', array('0', '1'));
    $huoniaoTag->assign('marryplanmealCheckNames', array('需要审核', '不需要审核'));
    $huoniaoTag->assign('marryplanmealCheckChecked', (int)$custommarryplanmealCheck);
    ...
    $huoniaoTag->assign('marryhotelfield_atlasMax', $custom_marryhotelfield_atlasMax);
    $huoniaoTag->assign('marryweddingcar_atlasMax', $custom_marryweddingcar_atlasMax);
    $huoniaoTag->assign('marryplancase_atlasMax', $custom_marryplancase_atlasMax);
    $huoniaoTag->assign('marryplanmeal_atlasMax', $custom_marryplanmeal_atlasMax);
