 ...
 * 管理婚宴菜单
 *
 * @version        $Id: marryhotelmenuList.php 2019-04-24 上午11:28:16 $
...
checkPurview("marryhotelmenuList");
$dsql  = new dsql($dbo);
$tpl   = dirname(__FILE__)."/../templates/marry";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "marryhotelmenuList.html";
...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `title`, `company`, `state`, `weight`, `pubdate`, `price` FROM `#@__".$tab."` WHERE 1 = 1".$where);
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.', "totalGray": '.$totalGray.', "totalAudit": '.$totalAudit.', "totalRefuse": '.$totalRefuse.'}, "hotelmenuList": '.json_encode($list).'}';
		}else{
...
//删除
	...
	if(!testPurview("marryhotelmenuDel")){
	...
	if($id != ""){
		...
		if(!empty($error)){
			...
			checkMarryHotelmenuCache($id);
			adminLog("删除婚宴菜单信息", join(", ", $title));
...
// 检查缓存
function checkMarryHotelmenuCache($id){
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/marry/marryhotelmenuList.js'
	);
