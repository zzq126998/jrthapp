...
if($dopost == "save" && $submit == "提交"){
	//保存到表
	$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `company`, `pics`, `price`, `duration`, `kilometre`, `click`, `pubdate`, `weight`, `state`) VALUES ('$title', '$comid', '$pics', '$price', '$duration', '$kilometre', '$click', '$pubdate', '$weight', '$state')");
	...
	if($submit == "提交"){
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `company` = '$comid', `pics` = '$pics', `price` = '$price', `duration` = '$duration', `kilometre` = '$kilometre', `click` = '$click', `weight` = '$weight', `state` = '$state' WHERE `id` = ".$id);
