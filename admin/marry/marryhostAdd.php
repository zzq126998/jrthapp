...
if($dopost == "save" && $submit == "提交"){
	//保存到表
	$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`hostname`, `company`, `photo`, `tel`, `price`, `note`, `click`, `pubdate`, `weight`, `state`) VALUES ('$hostname', '$comid', '$photo', '$tel', '$price', '$note', '$click', '$pubdate', '$weight', '$state')");
	...
	if($submit == "提交"){
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `hostname` = '$hostname', `company` = '$comid', `photo` = '$photo', `tel` = '$tel', `price` = '$price', `note` = '$note', `click` = '$click', `weight` = '$weight', `state` = '$state' WHERE `id` = ".$id);
