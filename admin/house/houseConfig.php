...
if ($type != "") {
    ...
    if ($type == "site") {
        ...
        $customagentCheck = (int)$agentCheck;
        ...
        $custom_houseCw_atlasMax = (int)$houseCw_atlasMax;
    ...
    //入驻费用
        ...
        //  $costArr[$key] = sprintf('%.2f', $value);
        // }
        // $customCost = serialize($costArr);

        // $custom_trialState = (int)$trialState;
        // $custom_trialAutoAudit = (int)$trialAutoAudit;

        $zjuserPriceCostArr = array();
        // print_r($trialCost);die;
        if($trialCost){
            $info = "";
            foreach ($trialCost as $key => $value) {
                $name = $value['name'];
                $list = $value['list'];
        ...
        if($info != ""){
            die('{"state": 200, "info": ' . json_encode($info) . '}');
        }
        // print_r($trialCost);die;
        $custom_zjuserPriceCost = serialize($trialCost);

        adminLog("修改房产经纪人套餐设置", "套餐费用");
    ...
    $customInc .= "\$customagentCheck = '" . $customagentCheck . "';\r\n";
    $customInc .= "\$custom_zjuserPriceCost = '".$custom_zjuserPriceCost."';\r\n";

    $customInc .= "\$custom_la_atlasMax = " . (int)$custom_la_atlasMax . ";\r\n";
    $customInc .= "\$custom_ll_atlasMax = " . (int)$custom_ll_atlasMax . ";\r\n";
    $customInc .= "\$custom_ca_atlasMax = " . (int)$custom_ca_atlasMax . ";\r\n";
    $customInc .= "\$custom_houseSale_atlasMax = " . (int)$custom_houseSale_atlasMax . ";\r\n";
    $customInc .= "\$custom_houseZu_atlasMax = " . (int)$custom_houseZu_atlasMax . ";\r\n";
    $customInc .= "\$custom_houseXzl_atlasMax = " . (int)$custom_houseXzl_atlasMax . ";\r\n";
    $customInc .= "\$custom_houseSp_atlasMax = " . (int)$custom_houseSp_atlasMax . ";\r\n";
    $customInc .= "\$custom_houseCf_atlasMax = " . (int)$custom_houseCf_atlasMax . ";\r\n";
    $customInc .= "\$custom_houseCw_atlasMax = " . (int)$custom_houseCw_atlasMax . ";\r\n";
...
if (file_exists($tpl . "/" . $templates)) {
    ...
    $huoniaoTag->assign('agentCheckNum', array('0', '1'));
    $huoniaoTag->assign('agentCheckNames', array('需要审核', '不需要审核'));
    $huoniaoTag->assign('agentCheckChecked', (int)$customagentCheck);
    ...
    $huoniaoTag->assign('houseCw_atlasMax', $custom_houseCw_atlasMax);
    ...
    $huoniaoTag->assign('zjuserPriceCost', $custom_zjuserPriceCost ? unserialize($custom_zjuserPriceCost) : array());
